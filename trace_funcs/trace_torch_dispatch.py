...
class LoggingMode(TorchDispatchMode):
    ...
    def __torch_dispatch__(self, func, types, args=(), kwargs=None):
        ...
        fmt_args = ", ".join(
            itertools.chain(
                (repr(tree_map(partial(self._fmt, with_type=True), a)) for a in args),
                (f"{k}={tree_map(partial(self._fmt, with_type=True), v)}" for k, v in kwargs.items()),
            )
        )
        fmt_rets = repr(tree_map(partial(self._fmt, with_type=True), rs))
        fmt_args = fmt_args.splitlines()[0]  # added by shifangxu, for aten.set_.source_Storage
        log_msg = f"Logging:{trace_line_num}:rank-{rank}:{fmt_rets} = {torch.overrides.resolve_name(func)}({fmt_args})"
