...
def _get_extra_te_kwargs(config: TransformerConfig):
    extra_transformer_engine_kwargs = {                                        # trace_info : t_8615
        "params_dtype": config.params_dtype,                                   # trace_info : t_8614
    }

    if _te_version >= packaging.version.Version("0.12.0"):                     # trace_info : t_8616
        if config.use_cpu_initialization:                                      # trace_info : t_8617
            extra_transformer_engine_kwargs["device"] = 'cpu'
        else:
            extra_transformer_engine_kwargs["device"] = torch.cuda.current_device()# trace_info : t_8618
    return extra_transformer_engine_kwargs                                     # trace_info : t_8619
...
class TENorm:
    ...
    def __new__(
        ...
        if config.normalization == "LayerNorm":                                # trace_info : t_8606
            instance = te.pytorch.LayerNorm(                                   # trace_info : t_8607, t_8612, t_8620
                hidden_size=hidden_size,                                       # trace_info : t_8608
                eps=eps,                                                       # trace_info : t_8609
                sequence_parallel=config.sequence_parallel,                    # trace_info : t_8610
                zero_centered_gamma=config.layernorm_zero_centered_gamma,      # trace_info : t_8611
                **_get_extra_te_kwargs(config),                                # trace_info : t_8613
            )
        ...
        return instance                                                        # trace_info : t_8621
