...
def compile_helpers():
    ...
    """
    import os                                                                  # trace_info : t_5520
    import subprocess                                                          # trace_info : t_5521

    command = ["make", "-C", os.path.abspath(os.path.dirname(__file__))]       # trace_info : t_5522
    if subprocess.run(command).returncode != 0:                                # trace_info : t_5523
        import sys
...
def log_single_rank(logger: logging.Logger, *args: Any, rank: int = 0, **kwargs: Any):
    ...
    """
    if torch.distributed.is_initialized():                                     # trace_info : t_13124, t_13159, t_13235, t_13261, t_13274, ...
        if torch.distributed.get_rank() == rank:                               # trace_info : t_13125, t_13160, t_13236, t_13262, t_13275, ...
            logger.log(*args, **kwargs)                                        # trace_info : t_13126, t_13161, t_13237, t_13263, t_13276, ...
    else:
...
def normalize(weights: List[float]) -> List[float]:
    ...
    """
    w = numpy.array(weights, dtype=numpy.float64)                              # trace_info : t_13090
    w_sum = numpy.sum(w)                                                       # trace_info : t_13091
    w = (w / w_sum).tolist()                                                   # trace_info : t_13092
    return w                                                                   # trace_info : t_13093
...
def get_blend_from_list(
    ...
    """
    if blend is None:                                                          # trace_info : t_13036, t_13043, t_13046, t_13049
        return None                                                            # trace_info : t_13044, t_13047, t_13050

    if len(blend) % 2 == 1:                                                    # trace_info : t_13037
        weight_per_dataset = None                                              # trace_info : t_13038
        raw_prefix_per_dataset = blend                                         # trace_info : t_13039
    else:
    ...
    prefix_per_dataset = [rppd.strip() for rppd in raw_prefix_per_dataset]     # trace_info : t_13040

    return prefix_per_dataset, weight_per_dataset                              # trace_info : t_13041
