...
class BlendedMegatronDatasetBuilder(object):
    ...
    def __init__(
        ...
        self.cls = cls                                                         # trace_info : t_13149
        self.sizes = sizes                                                     # trace_info : t_13150
        self.is_built_on_rank = is_built_on_rank                               # trace_info : t_13151
        self.config = config                                                   # trace_info : t_13152

        log_single_rank(                                                       # trace_info : t_13153, t_13158
            logger,                                                            # trace_info : t_13154
            logging.INFO,                                                      # trace_info : t_13155
            f"Building dataset splits with cls={cls.__name__}, sizes={self.sizes}, and config={self.config}",# trace_info : t_13156
        )

        if not self.config.mock:                                               # trace_info : t_13166
            for split in Split:                                                # trace_info : t_13167, t_13172, t_13177, t_13182
                size_is_none = self.sizes[split.value] is None                 # trace_info : t_13168, t_13173, t_13178
                if self.config.blend_per_split is None:                        # trace_info : t_13169, t_13174, t_13179
                    weights_are_none = self.config.blend[1] is None
                else:
                    if self.config.blend_per_split[split.value] is None:       # trace_info : t_13170, t_13175, t_13180
                        continue                                               # trace_info : t_13171, t_13176, t_13181
        ...
        if torch.distributed.is_initialized():                                 # trace_info : t_13183
            gb_rank = torch.distributed.get_rank()                             # trace_info : t_13184
            vp_rank = get_virtual_pipeline_model_parallel_rank()               # trace_info : t_13185
            if gb_rank == 0 and (vp_rank == 0 or vp_rank is None):             # trace_info : t_13187
                assert (                                                       # trace_info : t_13206
                    self.is_built_on_rank()                                    # trace_info : t_13188
    ...
    def build(self) -> List[Optional[TopLevelDataset]]:
        ...
        """
        datasets = self._build_blended_dataset_splits()                        # trace_info : t_13208

        for dataset in datasets:                                               # trace_info : t_14017, t_14021, t_14025, t_14029
            if dataset is not None and len(dataset) > 0:                       # trace_info : t_14018, t_14022, t_14026
                if isinstance(dataset, BlendedDataset):                        # trace_info : t_14020, t_14024, t_14028
                    # Check blend size
        ...
        return datasets                                                        # trace_info : t_14030
    ...
    def _build_blended_dataset_splits(self,) -> List[Optional[TopLevelDataset]]:
        ...
        ##
        if self.config.mock:                                                   # trace_info : t_13209
        ...
        ##
        elif self.config.blend:                                                # trace_info : t_13210
            prefixes, weights = self.config.blend                              # trace_info : t_13211
            if weights is not None:                                            # trace_info : t_13212
                weights = normalize(weights)

            split = self.config.split_matrix                                   # trace_info : t_13213

            # Blend consists of a single prefix
            if len(prefixes) == 1 and weights is None:                         # trace_info : t_13214
                return self._build_megatron_dataset_splits(prefixes[0], split, self.sizes)# trace_info : t_13215
    ...
    def _build_megatron_dataset_splits(
        ...
        low_level_dataset = self.cls.build_low_level_dataset(dataset_path, self.config)# trace_info : t_13216

        # Build the split indices for the low level dataset
        num_elements = self.cls.numel_low_level_dataset(low_level_dataset)     # trace_info : t_13349
        split_indices = []                                                     # trace_info : t_13352
        for i, _ in enumerate(Split):                                          # trace_info : t_13353, t_13358, t_13363, t_13368
            if split[i] is not None:                                           # trace_info : t_13354, t_13359, t_13364
                beg = int(round(split[i][0] * float(num_elements)))            # trace_info : t_13355, t_13360, t_13365
                end = int(round(split[i][1] * float(num_elements)))            # trace_info : t_13356, t_13361, t_13366
                split_indices.append(numpy.arange(start=beg, stop=end, step=1, dtype=numpy.int32))# trace_info : t_13357, t_13362, t_13367
            else:
                split_indices.append(None)

        # Build the mid level dataset
        mid_level_datasets = []                                                # trace_info : t_13369
        for i, _split in enumerate(Split):                                     # trace_info : t_13370, t_13585, t_13800, t_14015
            if split[i] is None:                                               # trace_info : t_13371, t_13586, t_13801
                mid_level_datasets.append(None)
            else:
                mid_level_datasets.append(                                     # trace_info : t_13372, t_13584, t_13587, t_13799, t_13802, ...
                    self.build_generic_dataset(                                # trace_info : t_13373, t_13383, t_13588, t_13598, t_13803, ...
                        self.cls,                                              # trace_info : t_13374, t_13589, t_13804
                        self.is_built_on_rank,                                 # trace_info : t_13375, t_13590, t_13805
                        synchronize_ranks,                                     # trace_info : t_13376, t_13591, t_13806
                        low_level_dataset,                                     # trace_info : t_13377, t_13592, t_13807
                        dataset_path,                                          # trace_info : t_13378, t_13593, t_13808
                        split_indices[i],                                      # trace_info : t_13379, t_13594, t_13809
                        sizes[i],                                              # trace_info : t_13380, t_13595, t_13810
                        _split,                                                # trace_info : t_13381, t_13596, t_13811
                        self.config,                                           # trace_info : t_13382, t_13597, t_13812
                    )
                )

        return mid_level_datasets                                              # trace_info : t_14016
    ...
    def build_generic_dataset(
        ...
        *args: Any,
        ...
        """
        if torch.distributed.is_initialized():                                 # trace_info : t_13384, t_13599, t_13814
            rank = torch.distributed.get_rank()                                # trace_info : t_13385, t_13600, t_13815

            dataset = None                                                     # trace_info : t_13386, t_13601, t_13816

            # First, build on rank 0
            if rank == 0 and is_built_on_rank():                               # trace_info : t_13387, t_13602, t_13817
                try:                                                           # trace_info : t_13405, t_13620, t_13835
                    dataset = cls(*args)                                       # trace_info : t_13406, t_13621, t_13836
            ...
            if synchronize_ranks:                                              # trace_info : t_13580, t_13795, t_14010
                torch.distributed.barrier()                                    # trace_info : t_13581, t_13796, t_14011

            # After, build on other ranks
            if rank != 0 and is_built_on_rank():                               # trace_info : t_13582, t_13797, t_14012
                dataset = cls(*args)

            return dataset                                                     # trace_info : t_13583, t_13798, t_14013
