...
class DType(Enum):
    ...
    def dtype_from_code(cls, value: int) -> Type[numpy.number]:
        ...
        """
        return getattr(numpy, cls(value).name)                                 # trace_info : t_13249
    ...
    def size(key: Union[int, Type[numpy.number]]) -> int:
        ...
        """
        if isinstance(key, int):                                               # trace_info : t_13251
            return DType.dtype_from_code(key)().itemsize
        elif numpy.number in key.__mro__:                                      # trace_info : t_13252
            return key().itemsize                                              # trace_info : t_13253
        else:
            raise ValueError
...
class _IndexReader(object):
    ...
    def __init__(self, idx_path: str, multimodal: bool) -> None:
        ...
        log_single_rank(logger, logging.INFO, f"Load the {type(self).__name__} from {idx_path}")# trace_info : t_13234

        with open(idx_path, "rb") as stream:                                   # trace_info : t_13242, t_13257
            header = stream.read(9)                                            # trace_info : t_13243
            assert header == _INDEX_HEADER, f"bad header, cannot read: {idx_path}"# trace_info : t_13244

            version = struct.unpack("<Q", stream.read(8))[0]                   # trace_info : t_13245
            assert version == 1, f"bad version, cannot read: {idx_path}"       # trace_info : t_13246

            code = struct.unpack("<B", stream.read(1))[0]                      # trace_info : t_13247
            self.dtype = DType.dtype_from_code(code)                           # trace_info : t_13248
            self.dtype_size = DType.size(self.dtype)                           # trace_info : t_13250

            self.sequence_count = struct.unpack("<Q", stream.read(8))[0]       # trace_info : t_13254
            self.document_count = struct.unpack("<Q", stream.read(8))[0]       # trace_info : t_13255

            offset = stream.tell()                                             # trace_info : t_13256

        self.bin_buffer_mmap = numpy.memmap(idx_path, mode="r", order="C")     # trace_info : t_13258
        self.bin_buffer = memoryview(self.bin_buffer_mmap)                     # trace_info : t_13259

        log_single_rank(logger, logging.INFO, f"\tExtract the sequence lengths")# trace_info : t_13260
        t_beg = time.time()                                                    # trace_info : t_13268
        self.sequence_lengths = numpy.frombuffer(                              # trace_info : t_13269, t_13271
            self.bin_buffer, dtype=numpy.int32, count=self.sequence_count, offset=offset# trace_info : t_13270
        )
        t_end = time.time()                                                    # trace_info : t_13272
        log_single_rank(logger, logging.DEBUG, f"\t> time elapsed: {t_end - t_beg:4f} seconds")# trace_info : t_13273

        log_single_rank(logger, logging.INFO, f"\tExtract the sequence pointers")# trace_info : t_13277
        t_beg = time.time()                                                    # trace_info : t_13285
        self.sequence_pointers = numpy.frombuffer(                             # trace_info : t_13286, t_13291
            self.bin_buffer,                                                   # trace_info : t_13287
            dtype=numpy.int64,                                                 # trace_info : t_13288
            count=self.sequence_count,                                         # trace_info : t_13289
            offset=offset + self.sequence_lengths.nbytes,                      # trace_info : t_13290
        )
        t_end = time.time()                                                    # trace_info : t_13292
        log_single_rank(logger, logging.DEBUG, f"\t> time elapsed: {t_end - t_beg:4f} seconds")# trace_info : t_13293

        log_single_rank(logger, logging.INFO, f"\tExtract the document indices")# trace_info : t_13297
        t_beg = time.time()                                                    # trace_info : t_13305
        self.document_indices = numpy.frombuffer(                              # trace_info : t_13306, t_13311
            self.bin_buffer,                                                   # trace_info : t_13307
            dtype=numpy.int64,                                                 # trace_info : t_13308
            count=self.document_count,                                         # trace_info : t_13309
            offset=offset + self.sequence_lengths.nbytes + self.sequence_pointers.nbytes,# trace_info : t_13310
        )
        t_end = time.time()                                                    # trace_info : t_13312
        log_single_rank(logger, logging.DEBUG, f"\t> time elapsed: {t_end - t_beg:4f} seconds")# trace_info : t_13313

        self.sequence_modes = None                                             # trace_info : t_13317
        if multimodal:                                                         # trace_info : t_13318
        ...
        assert self.sequence_lengths.shape[0] == len(self)                     # trace_info : t_13319
        assert self.sequence_lengths.shape[0] == self.sequence_count           # trace_info : t_13321
        assert self.sequence_lengths.shape[0] == self.document_indices[-1]     # trace_info : t_13322

        log_single_rank(logger, logging.INFO, f"> total number of sequences: {len(self)}")# trace_info : t_13323
        log_single_rank(                                                       # trace_info : t_13332, t_13336
            logger,                                                            # trace_info : t_13333
            logging.INFO,                                                      # trace_info : t_13334
            f"> total number of documents: {self.document_indices.shape[0] - 1}",# trace_info : t_13335
        )

    def __del__(self) -> None:
        """Clean up the object
        """
        if hasattr(self, "bin_buffer_mmap"):                                   # trace_info : t_25427
            self.bin_buffer_mmap._mmap.close()                                 # trace_info : t_25428
            del self.bin_buffer_mmap                                           # trace_info : t_25429
    ...
    def __len__(self) -> int:
        ...
        """
        return self.sequence_count                                             # trace_info : t_13320, t_13324
...
class IndexedDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, path_prefix: str, multimodal: bool = False, mmap: bool = True) -> None:
        super().__init__()                                                     # trace_info : t_13218
        self.path_prefix = None                                                # trace_info : t_13219
        self.multimodal = None                                                 # trace_info : t_13220
        self.mmap = None                                                       # trace_info : t_13221

        self.initialize(path_prefix, multimodal, mmap)                         # trace_info : t_13222
    ...
    def initialize(self, path_prefix: str, multimodal: bool, mmap: bool) -> None:
        ...
        """
        idx_path = get_idx_path(path_prefix)                                   # trace_info : t_13223
        bin_path = get_bin_path(path_prefix)                                   # trace_info : t_13225
        assert os.path.exists(idx_path) and os.path.exists(                    # trace_info : t_13227, t_13229
            bin_path                                                           # trace_info : t_13228
        ), f"One or both of the .idx and .bin files cannot be found at the path prefix {path_prefix}"

        self.path_prefix = path_prefix                                         # trace_info : t_13230
        self.multimodal = multimodal                                           # trace_info : t_13231
        self.mmap = mmap                                                       # trace_info : t_13232

        self.index = _IndexReader(idx_path, self.multimodal)                   # trace_info : t_13233
        self.bin_buffer = None                                                 # trace_info : t_13344
        self.bin_buffer_mmap = None                                            # trace_info : t_13345
        if mmap:                                                               # trace_info : t_13346
            self.bin_buffer_mmap = numpy.memmap(bin_path, mode="r", order="C") # trace_info : t_13347
            self.bin_buffer = memoryview(self.bin_buffer_mmap)                 # trace_info : t_13348
    ...
    def __getstate__(self) -> Tuple[str, bool, bool]:
        ...
        """
        return self.path_prefix, self.multimodal, self.mmap                    # trace_info : t_14203, t_14204, t_14245, t_14246, t_14287, ...
    ...
    def __del__(self) -> None:
        """Clean up the object
        """
        if self.bin_buffer_mmap is not None:                                   # trace_info : t_25423
            self.bin_buffer_mmap._mmap.close()                                 # trace_info : t_25424
        del self.bin_buffer_mmap                                               # trace_info : t_25425
        del self.index                                                         # trace_info : t_25426
    ...
    def sequence_lengths(self) -> numpy.ndarray:
        ...
        """
        return self.index.sequence_lengths                                     # trace_info : t_13351
...
def get_idx_path(path_prefix: str) -> str:
    ...
    """
    return path_prefix + ".idx"                                                # trace_info : t_13224
...
def get_bin_path(path_prefix: str) -> str:
    ...
    """
    return path_prefix + ".bin"                                                # trace_info : t_13226
