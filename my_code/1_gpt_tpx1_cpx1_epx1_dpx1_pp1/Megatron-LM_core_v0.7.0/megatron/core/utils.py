...
def ensure_divisibility(numerator, denominator):
    ...
    assert numerator % denominator == 0, "{} is not divisible by {}".format(numerator, denominator)# trace_info : t_6460, t_6729, t_6733, t_6737, t_6778, ...


def divide(numerator, denominator):
    """Ensure that numerator is divisible by the denominator and return
    the division value."""
    ensure_divisibility(numerator, denominator)                                # trace_info : t_6459, t_6728, t_6732, t_6736, t_6777, ...
    return numerator // denominator                                            # trace_info : t_6461, t_6730, t_6734, t_6738, t_6779, ...
...
def get_attr_wrapped_model(model, attr, allow_none=True, return_model_obj=False):
    ...
    if isinstance(model, list):                                                # trace_info : t_9153, t_12934, t_14825, t_14845, t_14989, ...
        raise RuntimeError("_get_attr_wrapped_model given a list of models")

    if allow_none:                                                             # trace_info : t_9154, t_12935, t_14826, t_14846, t_14990, ...

        def condition(model, attr):                                            # trace_info : t_14847, t_14991, t_15013, t_16496, t_18488, ...
            return not hasattr(model, attr)                                    # trace_info : t_14849, t_14853, t_14857, t_14993, t_14997, ...

    else:

        def condition(model, attr):                                            # trace_info : t_9155, t_12936, t_14827, t_16597, t_18468, ...
            return getattr(model, attr, None) is None                          # trace_info : t_9157, t_9161, t_12938, t_14829, t_16599, ...

    while condition(model, attr):                                              # trace_info : t_9156, t_9160, t_12937, t_14828, t_14848, ...
        if not hasattr(model, "module"):                                       # trace_info : t_9158, t_14850, t_14854, t_14994, t_14998, ...
            raise RuntimeError(f"_get_attr_wrapped_model couldn't find attribute {attr}")

        model = model.module                                                   # trace_info : t_9159, t_14851, t_14855, t_14995, t_14999, ...

    if return_model_obj:                                                       # trace_info : t_9162, t_12939, t_14830, t_14858, t_15002, ...
        return model
    return getattr(model, attr)                                                # trace_info : t_9163, t_12940, t_14831, t_14859, t_15003, ...


def get_model_type(model):
    return get_attr_wrapped_model(model, 'model_type')                         # trace_info : t_14844, t_16493, t_18485, t_20132, t_22124, ...


def get_model_config(model):
    return get_attr_wrapped_model(model, 'config', allow_none=False)           # trace_info : t_9152, t_12933, t_14824, t_16594, t_18465, ...
...
class GlobalMemoryBuffer:
    ...
    def __init__(self):
        self.buffer = {}                                                       # trace_info : t_5370

    def get_tensor(self, tensor_shape, dtype, name):
        required_len = reduce(operator.mul, tensor_shape, 1)                   # trace_info : t_15417, t_15920, t_19058, t_19559, t_22697, ...
        if (
            self.buffer.get((name, dtype), None) is None                       # trace_info : t_15418, t_15921, t_19059, t_19560, t_22698, ...
            or self.buffer[(name, dtype)].numel() < required_len               # trace_info : t_15922, t_19060, t_19561, t_22699, t_23200
        ):
            self.buffer[(name, dtype)] = torch.empty(                          # trace_info : t_15419, t_15421
                required_len, dtype=dtype, device=torch.cuda.current_device(), requires_grad=False# trace_info : t_15420
            )

        return self.buffer[(name, dtype)][0:required_len].view(*tensor_shape)  # trace_info : t_15422, t_15923, t_19061, t_19562, t_22700, ...
...
def _kernel_make_viewless_tensor(inp, requires_grad):
    ...
    field.
    '''
    out = torch.empty((1,), dtype=inp.dtype, device=inp.device, requires_grad=requires_grad,)# trace_info : t_15286, t_15611, t_15789, t_16112, t_18927, ...
    out.data = inp.data                                                        # trace_info : t_15287, t_15612, t_15790, t_16113, t_18928, ...
    return out                                                                 # trace_info : t_15288, t_15613, t_15791, t_16114, t_18929, ...
...
class MakeViewlessTensor(torch.autograd.Function):
    ...
    def forward(ctx, inp, requires_grad):
        return _kernel_make_viewless_tensor(inp, requires_grad)                # trace_info : t_15285, t_15610, t_15788, t_16111, t_18926, ...
...
def make_viewless_tensor(inp, requires_grad, keep_graph):
    ...
    if inp._base is None:                                                      # trace_info : t_15251, t_15282, t_15607, t_15753, t_15785, ...
        return inp                                                             # trace_info : t_15252, t_15754, t_16255, t_18893, t_19393, ...

    # create viewless tensor
    if keep_graph:                                                             # trace_info : t_15283, t_15608, t_15786, t_16109, t_18924, ...
        return MakeViewlessTensor.apply(inp, requires_grad)                    # trace_info : t_15284, t_15609, t_15787, t_16110, t_18925, ...
    else:
...
def init_method_normal(sigma):
    ...
    def init_(tensor):                                                         # trace_info : t_6314
        return torch.nn.init.normal_(tensor, mean=0.0, std=sigma)              # trace_info : t_6518, t_6538, t_7071, t_7400, t_8073, ...

    return init_                                                               # trace_info : t_6315


def scaled_init_method_normal(sigma, num_layers):
    """Init method based on N(0, sigma/sqrt(2*num_layers)."""
    std = sigma / math.sqrt(2.0 * num_layers)                                  # trace_info : t_6320

    def init_(tensor):                                                         # trace_info : t_6321
        return torch.nn.init.normal_(tensor, mean=0.0, std=std)                # trace_info : t_6929, t_7558, t_7931, t_8560

    return init_                                                               # trace_info : t_6322
...
class StragglerDetector:
    ...
    def null_method(self) -> None:
        ...
        pass                                                                   # trace_info : t_15052, t_15150, t_15162, t_16434, t_18693, ...
    ...
    def __enter__(self) -> "StragglerDetector":
        ...
        """
        self.start()                                                           # trace_info : t_15051, t_15161, t_18692, t_18802, t_22331, ...
        return self                                                            # trace_info : t_15053, t_15163, t_18694, t_18804, t_22333, ...
    ...
    def __call__(self, bdata: bool = False) -> "StragglerDetector":
        ...
        Returns:
            StragglerDetector: the instance
        """
        self.bdata = bdata                                                     # trace_info : t_15049, t_18690, t_22329
        return self                                                            # trace_info : t_15050, t_18691, t_22330
    ...
    def __exit__(
        ...
        ret = False                                                            # trace_info : t_15147, t_16431, t_18788, t_20070, t_22427, ...
        if ex_type is not None:                                                # trace_info : t_15148, t_16432, t_18789, t_20071, t_22428, ...
            err = traceback.format_exception(ex_tb)
            logger.warning(f"{str(ex_val)}\n{err}")
            ret = True
        self.stop()                                                            # trace_info : t_15149, t_16433, t_18790, t_20072, t_22429, ...
        return ret                                                             # trace_info : t_15151, t_16435, t_18792, t_20074, t_22431, ...
