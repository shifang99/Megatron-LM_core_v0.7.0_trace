...
class LanguageModule(MegatronModule):
    ...
    def __init__(self, config: TransformerConfig) -> None:
        super().__init__(config=config)                                        # trace_info : t_6405
    ...
    def compute_language_model_loss(self, labels: Tensor, logits: Tensor) -> Tensor:
        ...
        """
        # [b s] => [s b]
        labels = labels.transpose(0, 1).contiguous()                           # trace_info : t_16336, t_19975, t_23614
        loss = tensor_parallel.vocab_parallel_cross_entropy(logits.float(), labels)# trace_info : t_16337, t_19976, t_23615

        # [s b] => [b, s]
        loss = loss.transpose(0, 1).contiguous()                               # trace_info : t_16401, t_20040, t_23679
        return loss                                                            # trace_info : t_16402, t_20041, t_23680
    ...
    def setup_embeddings_and_output_layer(self) -> None:
        ...
        if self.pre_process:                                                   # trace_info : t_8685
            self.embedding.word_embeddings.weight.is_embedding_or_output_parameter = True# trace_info : t_8686
        if self.post_process and self.output_layer.weight is not None:         # trace_info : t_8687
            self.output_layer.weight.is_embedding_or_output_parameter = True

        if not self.share_embeddings_and_output_weights:                       # trace_info : t_8688
            return

        if self.pre_process and self.post_process:                             # trace_info : t_8689
            # Zero out wgrad if sharing embeddings between two layers on same
            # pipeline stage to make sure grad accumulation into main_grad is
            # correct and does not include garbage values (e.g., from torch.empty).
            self.shared_embedding_or_output_weight().zero_out_wgrad = True     # trace_info : t_8690
            return                                                             # trace_info : t_8693
    ...
    def shared_embedding_or_output_weight(self) -> Tensor:
        ...
        """
        if self.pre_process:                                                   # trace_info : t_8691, t_16272, t_19911, t_23550
            return self.embedding.word_embeddings.weight                       # trace_info : t_8692, t_16273, t_19912, t_23551
