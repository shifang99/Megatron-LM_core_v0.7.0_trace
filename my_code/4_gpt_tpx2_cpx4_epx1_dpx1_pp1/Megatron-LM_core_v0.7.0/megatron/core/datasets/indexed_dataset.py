...
class DType(Enum):
    ...
    def dtype_from_code(cls, value: int) -> Type[numpy.number]:
        ...
        """
        return getattr(numpy, cls(value).name)                                 # trace_info : t_16108
    ...
    def size(key: Union[int, Type[numpy.number]]) -> int:
        ...
        """
        if isinstance(key, int):                                               # trace_info : t_16110
            return DType.dtype_from_code(key)().itemsize
        elif numpy.number in key.__mro__:                                      # trace_info : t_16111
            return key().itemsize                                              # trace_info : t_16112
        else:
            raise ValueError
...
class _IndexReader(object):
    ...
    def __init__(self, idx_path: str, multimodal: bool) -> None:
        ...
        log_single_rank(logger, logging.INFO, f"Load the {type(self).__name__} from {idx_path}")# trace_info : t_16093

        with open(idx_path, "rb") as stream:                                   # trace_info : t_16101, t_16116
            header = stream.read(9)                                            # trace_info : t_16102
            assert header == _INDEX_HEADER, f"bad header, cannot read: {idx_path}"# trace_info : t_16103

            version = struct.unpack("<Q", stream.read(8))[0]                   # trace_info : t_16104
            assert version == 1, f"bad version, cannot read: {idx_path}"       # trace_info : t_16105

            code = struct.unpack("<B", stream.read(1))[0]                      # trace_info : t_16106
            self.dtype = DType.dtype_from_code(code)                           # trace_info : t_16107
            self.dtype_size = DType.size(self.dtype)                           # trace_info : t_16109

            self.sequence_count = struct.unpack("<Q", stream.read(8))[0]       # trace_info : t_16113
            self.document_count = struct.unpack("<Q", stream.read(8))[0]       # trace_info : t_16114

            offset = stream.tell()                                             # trace_info : t_16115

        self.bin_buffer_mmap = numpy.memmap(idx_path, mode="r", order="C")     # trace_info : t_16117
        self.bin_buffer = memoryview(self.bin_buffer_mmap)                     # trace_info : t_16118

        log_single_rank(logger, logging.INFO, f"\tExtract the sequence lengths")# trace_info : t_16119
        t_beg = time.time()                                                    # trace_info : t_16127
        self.sequence_lengths = numpy.frombuffer(                              # trace_info : t_16128, t_16130
            self.bin_buffer, dtype=numpy.int32, count=self.sequence_count, offset=offset# trace_info : t_16129
        )
        t_end = time.time()                                                    # trace_info : t_16131
        log_single_rank(logger, logging.DEBUG, f"\t> time elapsed: {t_end - t_beg:4f} seconds")# trace_info : t_16132

        log_single_rank(logger, logging.INFO, f"\tExtract the sequence pointers")# trace_info : t_16136
        t_beg = time.time()                                                    # trace_info : t_16144
        self.sequence_pointers = numpy.frombuffer(                             # trace_info : t_16145, t_16150
            self.bin_buffer,                                                   # trace_info : t_16146
            dtype=numpy.int64,                                                 # trace_info : t_16147
            count=self.sequence_count,                                         # trace_info : t_16148
            offset=offset + self.sequence_lengths.nbytes,                      # trace_info : t_16149
        )
        t_end = time.time()                                                    # trace_info : t_16151
        log_single_rank(logger, logging.DEBUG, f"\t> time elapsed: {t_end - t_beg:4f} seconds")# trace_info : t_16152

        log_single_rank(logger, logging.INFO, f"\tExtract the document indices")# trace_info : t_16156
        t_beg = time.time()                                                    # trace_info : t_16164
        self.document_indices = numpy.frombuffer(                              # trace_info : t_16165, t_16170
            self.bin_buffer,                                                   # trace_info : t_16166
            dtype=numpy.int64,                                                 # trace_info : t_16167
            count=self.document_count,                                         # trace_info : t_16168
            offset=offset + self.sequence_lengths.nbytes + self.sequence_pointers.nbytes,# trace_info : t_16169
        )
        t_end = time.time()                                                    # trace_info : t_16171
        log_single_rank(logger, logging.DEBUG, f"\t> time elapsed: {t_end - t_beg:4f} seconds")# trace_info : t_16172

        self.sequence_modes = None                                             # trace_info : t_16176
        if multimodal:                                                         # trace_info : t_16177
        ...
        assert self.sequence_lengths.shape[0] == len(self)                     # trace_info : t_16178
        assert self.sequence_lengths.shape[0] == self.sequence_count           # trace_info : t_16180
        assert self.sequence_lengths.shape[0] == self.document_indices[-1]     # trace_info : t_16181

        log_single_rank(logger, logging.INFO, f"> total number of sequences: {len(self)}")# trace_info : t_16182
        log_single_rank(                                                       # trace_info : t_16191, t_16195
            logger,                                                            # trace_info : t_16192
            logging.INFO,                                                      # trace_info : t_16193
            f"> total number of documents: {self.document_indices.shape[0] - 1}",# trace_info : t_16194
        )

    def __del__(self) -> None:
        """Clean up the object
        """
        if hasattr(self, "bin_buffer_mmap"):                                   # trace_info : t_26924
            self.bin_buffer_mmap._mmap.close()                                 # trace_info : t_26925
            del self.bin_buffer_mmap                                           # trace_info : t_26926
    ...
    def __len__(self) -> int:
        ...
        """
        return self.sequence_count                                             # trace_info : t_16179, t_16183
...
class IndexedDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, path_prefix: str, multimodal: bool = False, mmap: bool = True) -> None:
        super().__init__()                                                     # trace_info : t_16077
        self.path_prefix = None                                                # trace_info : t_16078
        self.multimodal = None                                                 # trace_info : t_16079
        self.mmap = None                                                       # trace_info : t_16080

        self.initialize(path_prefix, multimodal, mmap)                         # trace_info : t_16081
    ...
    def initialize(self, path_prefix: str, multimodal: bool, mmap: bool) -> None:
        ...
        """
        idx_path = get_idx_path(path_prefix)                                   # trace_info : t_16082
        bin_path = get_bin_path(path_prefix)                                   # trace_info : t_16084
        assert os.path.exists(idx_path) and os.path.exists(                    # trace_info : t_16086, t_16088
            bin_path                                                           # trace_info : t_16087
        ), f"One or both of the .idx and .bin files cannot be found at the path prefix {path_prefix}"

        self.path_prefix = path_prefix                                         # trace_info : t_16089
        self.multimodal = multimodal                                           # trace_info : t_16090
        self.mmap = mmap                                                       # trace_info : t_16091

        self.index = _IndexReader(idx_path, self.multimodal)                   # trace_info : t_16092
        self.bin_buffer = None                                                 # trace_info : t_16203
        self.bin_buffer_mmap = None                                            # trace_info : t_16204
        if mmap:                                                               # trace_info : t_16205
            self.bin_buffer_mmap = numpy.memmap(bin_path, mode="r", order="C") # trace_info : t_16206
            self.bin_buffer = memoryview(self.bin_buffer_mmap)                 # trace_info : t_16207
    ...
    def __getstate__(self) -> Tuple[str, bool, bool]:
        ...
        """
        return self.path_prefix, self.multimodal, self.mmap                    # trace_info : t_17062, t_17063, t_17104, t_17105, t_17146, ...
    ...
    def __del__(self) -> None:
        """Clean up the object
        """
        if self.bin_buffer_mmap is not None:                                   # trace_info : t_26920
            self.bin_buffer_mmap._mmap.close()                                 # trace_info : t_26921
        del self.bin_buffer_mmap                                               # trace_info : t_26922
        del self.index                                                         # trace_info : t_26923
    ...
    def sequence_lengths(self) -> numpy.ndarray:
        ...
        """
        return self.index.sequence_lengths                                     # trace_info : t_16210
...
def get_idx_path(path_prefix: str) -> str:
    ...
    """
    return path_prefix + ".idx"                                                # trace_info : t_16083
...
def get_bin_path(path_prefix: str) -> str:
    ...
    """
    return path_prefix + ".bin"                                                # trace_info : t_16085
