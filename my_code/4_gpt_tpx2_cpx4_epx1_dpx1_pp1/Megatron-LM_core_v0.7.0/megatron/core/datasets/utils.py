...
def compile_helpers():
    ...
    """
    import os                                                                  # trace_info : t_8830
    import subprocess                                                          # trace_info : t_8831

    command = ["make", "-C", os.path.abspath(os.path.dirname(__file__))]       # trace_info : t_8832
    if subprocess.run(command).returncode != 0:                                # trace_info : t_8833
        import sys
...
def log_single_rank(logger: logging.Logger, *args: Any, rank: int = 0, **kwargs: Any):
    ...
    """
    if torch.distributed.is_initialized():                                     # trace_info : t_15983, t_16018, t_16094, t_16120, t_16133, ...
        if torch.distributed.get_rank() == rank:                               # trace_info : t_15984, t_16019, t_16095, t_16121, t_16134, ...
            logger.log(*args, **kwargs)                                        # trace_info : t_15985, t_16020, t_16096, t_16122, t_16135, ...
    else:
...
def normalize(weights: List[float]) -> List[float]:
    ...
    """
    w = numpy.array(weights, dtype=numpy.float64)                              # trace_info : t_15949
    w_sum = numpy.sum(w)                                                       # trace_info : t_15950
    w = (w / w_sum).tolist()                                                   # trace_info : t_15951
    return w                                                                   # trace_info : t_15952
...
def get_blend_from_list(
    ...
    """
    if blend is None:                                                          # trace_info : t_15895, t_15902, t_15905, t_15908
        return None                                                            # trace_info : t_15903, t_15906, t_15909

    if len(blend) % 2 == 1:                                                    # trace_info : t_15896
        weight_per_dataset = None                                              # trace_info : t_15897
        raw_prefix_per_dataset = blend                                         # trace_info : t_15898
    else:
    ...
    prefix_per_dataset = [rppd.strip() for rppd in raw_prefix_per_dataset]     # trace_info : t_15899

    return prefix_per_dataset, weight_per_dataset                              # trace_info : t_15900
