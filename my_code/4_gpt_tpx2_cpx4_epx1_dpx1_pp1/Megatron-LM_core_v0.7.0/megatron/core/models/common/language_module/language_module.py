...
class LanguageModule(MegatronModule):
    ...
    def __init__(self, config: TransformerConfig) -> None:
        super().__init__(config=config)                                        # trace_info : t_9711
    ...
    def compute_language_model_loss(self, labels: Tensor, logits: Tensor) -> Tensor:
        ...
        """
        # [b s] => [s b]
        labels = labels.transpose(0, 1).contiguous()                           # trace_info : t_18730, t_21916, t_25102
        loss = tensor_parallel.vocab_parallel_cross_entropy(logits.float(), labels)# trace_info : t_18731, t_21917, t_25103

        # [s b] => [b, s]
        loss = loss.transpose(0, 1).contiguous()                               # trace_info : t_18795, t_21981, t_25167
        return loss                                                            # trace_info : t_18796, t_21982, t_25168
    ...
    def setup_embeddings_and_output_layer(self) -> None:
        ...
        if self.pre_process:                                                   # trace_info : t_11544
            self.embedding.word_embeddings.weight.is_embedding_or_output_parameter = True# trace_info : t_11545
        if self.post_process and self.output_layer.weight is not None:         # trace_info : t_11546
            self.output_layer.weight.is_embedding_or_output_parameter = True

        if not self.share_embeddings_and_output_weights:                       # trace_info : t_11547
            return

        if self.pre_process and self.post_process:                             # trace_info : t_11548
            # Zero out wgrad if sharing embeddings between two layers on same
            # pipeline stage to make sure grad accumulation into main_grad is
            # correct and does not include garbage values (e.g., from torch.empty).
            self.shared_embedding_or_output_weight().zero_out_wgrad = True     # trace_info : t_11549
            return                                                             # trace_info : t_11552
    ...
    def shared_embedding_or_output_weight(self) -> Tensor:
        ...
        """
        if self.pre_process:                                                   # trace_info : t_11550, t_18674, t_21860, t_25046
            return self.embedding.word_embeddings.weight                       # trace_info : t_11551, t_18675, t_21861, t_25047
