...
def get_default_strategy(action: StrategyAction, backend: str, version: int):
    ...
    try:                                                                       # trace_info : t_31423, t_99016
        if backend == 'zarr':                                                  # trace_info : t_31424, t_99017
        ...
        elif backend == 'torch_dist':                                          # trace_info : t_31425, t_99018
            error_hint = ' Please use PyTorch version >=2.1'                   # trace_info : t_31426, t_99019
            from .torch import _import_trigger                                 # trace_info : t_31427, t_99020
    except ImportError as e:
        raise CheckpointingException(
            f'Cannot import a default strategy for: {(action.value, backend, version)}. Error: {e}. Hint: {error_hint}'
        ) from e
    try:                                                                       # trace_info : t_31642, t_99021
        return default_strategies[action.value][(backend, version)]            # trace_info : t_31643, t_99022
...
class SaveStrategyBase(ABC):
    ...
    def __init__(self, backend: str, version: int):
        self.backend = backend                                                 # trace_info : t_31635, t_31654
        self.version = version                                                 # trace_info : t_31636, t_31655
...
class AsyncSaveShardedStrategy(SaveShardedStrategy):
    ...
    def save(self, sharded_state_dict: ShardedStateDict, checkpoint_dir: Path):
        ...
        async_request = self.async_save(sharded_state_dict, checkpoint_dir)    # trace_info : t_86125, t_153492
        async_request.execute_sync()                                           # trace_info : t_88674, t_156041
