...
class DType(Enum):
    ...
    def dtype_from_code(cls, value: int) -> Type[numpy.number]:
        ...
        """
        return getattr(numpy, cls(value).name)                                 # trace_info : t_16278
    ...
    def size(key: Union[int, Type[numpy.number]]) -> int:
        ...
        """
        if isinstance(key, int):                                               # trace_info : t_16280
            return DType.dtype_from_code(key)().itemsize
        elif numpy.number in key.__mro__:                                      # trace_info : t_16281
            return key().itemsize                                              # trace_info : t_16282
        else:
            raise ValueError
...
class _IndexReader(object):
    ...
    def __init__(self, idx_path: str, multimodal: bool) -> None:
        ...
        log_single_rank(logger, logging.INFO, f"Load the {type(self).__name__} from {idx_path}")# trace_info : t_16263

        with open(idx_path, "rb") as stream:                                   # trace_info : t_16271, t_16286
            header = stream.read(9)                                            # trace_info : t_16272
            assert header == _INDEX_HEADER, f"bad header, cannot read: {idx_path}"# trace_info : t_16273

            version = struct.unpack("<Q", stream.read(8))[0]                   # trace_info : t_16274
            assert version == 1, f"bad version, cannot read: {idx_path}"       # trace_info : t_16275

            code = struct.unpack("<B", stream.read(1))[0]                      # trace_info : t_16276
            self.dtype = DType.dtype_from_code(code)                           # trace_info : t_16277
            self.dtype_size = DType.size(self.dtype)                           # trace_info : t_16279

            self.sequence_count = struct.unpack("<Q", stream.read(8))[0]       # trace_info : t_16283
            self.document_count = struct.unpack("<Q", stream.read(8))[0]       # trace_info : t_16284

            offset = stream.tell()                                             # trace_info : t_16285

        self.bin_buffer_mmap = numpy.memmap(idx_path, mode="r", order="C")     # trace_info : t_16287
        self.bin_buffer = memoryview(self.bin_buffer_mmap)                     # trace_info : t_16288

        log_single_rank(logger, logging.INFO, f"\tExtract the sequence lengths")# trace_info : t_16289
        t_beg = time.time()                                                    # trace_info : t_16297
        self.sequence_lengths = numpy.frombuffer(                              # trace_info : t_16298, t_16300
            self.bin_buffer, dtype=numpy.int32, count=self.sequence_count, offset=offset# trace_info : t_16299
        )
        t_end = time.time()                                                    # trace_info : t_16301
        log_single_rank(logger, logging.DEBUG, f"\t> time elapsed: {t_end - t_beg:4f} seconds")# trace_info : t_16302

        log_single_rank(logger, logging.INFO, f"\tExtract the sequence pointers")# trace_info : t_16306
        t_beg = time.time()                                                    # trace_info : t_16314
        self.sequence_pointers = numpy.frombuffer(                             # trace_info : t_16315, t_16320
            self.bin_buffer,                                                   # trace_info : t_16316
            dtype=numpy.int64,                                                 # trace_info : t_16317
            count=self.sequence_count,                                         # trace_info : t_16318
            offset=offset + self.sequence_lengths.nbytes,                      # trace_info : t_16319
        )
        t_end = time.time()                                                    # trace_info : t_16321
        log_single_rank(logger, logging.DEBUG, f"\t> time elapsed: {t_end - t_beg:4f} seconds")# trace_info : t_16322

        log_single_rank(logger, logging.INFO, f"\tExtract the document indices")# trace_info : t_16326
        t_beg = time.time()                                                    # trace_info : t_16334
        self.document_indices = numpy.frombuffer(                              # trace_info : t_16335, t_16340
            self.bin_buffer,                                                   # trace_info : t_16336
            dtype=numpy.int64,                                                 # trace_info : t_16337
            count=self.document_count,                                         # trace_info : t_16338
            offset=offset + self.sequence_lengths.nbytes + self.sequence_pointers.nbytes,# trace_info : t_16339
        )
        t_end = time.time()                                                    # trace_info : t_16341
        log_single_rank(logger, logging.DEBUG, f"\t> time elapsed: {t_end - t_beg:4f} seconds")# trace_info : t_16342

        self.sequence_modes = None                                             # trace_info : t_16346
        if multimodal:                                                         # trace_info : t_16347
        ...
        assert self.sequence_lengths.shape[0] == len(self)                     # trace_info : t_16348
        assert self.sequence_lengths.shape[0] == self.sequence_count           # trace_info : t_16350
        assert self.sequence_lengths.shape[0] == self.document_indices[-1]     # trace_info : t_16351

        log_single_rank(logger, logging.INFO, f"> total number of sequences: {len(self)}")# trace_info : t_16352
        log_single_rank(                                                       # trace_info : t_16361, t_16365
            logger,                                                            # trace_info : t_16362
            logging.INFO,                                                      # trace_info : t_16363
            f"> total number of documents: {self.document_indices.shape[0] - 1}",# trace_info : t_16364
        )

    def __del__(self) -> None:
        """Clean up the object
        """
        if hasattr(self, "bin_buffer_mmap"):                                   # trace_info : t_156141
            self.bin_buffer_mmap._mmap.close()                                 # trace_info : t_156142
            del self.bin_buffer_mmap                                           # trace_info : t_156143
    ...
    def __len__(self) -> int:
        ...
        """
        return self.sequence_count                                             # trace_info : t_16349, t_16353
...
class IndexedDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, path_prefix: str, multimodal: bool = False, mmap: bool = True) -> None:
        super().__init__()                                                     # trace_info : t_16247
        self.path_prefix = None                                                # trace_info : t_16248
        self.multimodal = None                                                 # trace_info : t_16249
        self.mmap = None                                                       # trace_info : t_16250

        self.initialize(path_prefix, multimodal, mmap)                         # trace_info : t_16251
    ...
    def initialize(self, path_prefix: str, multimodal: bool, mmap: bool) -> None:
        ...
        """
        idx_path = get_idx_path(path_prefix)                                   # trace_info : t_16252
        bin_path = get_bin_path(path_prefix)                                   # trace_info : t_16254
        assert os.path.exists(idx_path) and os.path.exists(                    # trace_info : t_16256, t_16258
            bin_path                                                           # trace_info : t_16257
        ), f"One or both of the .idx and .bin files cannot be found at the path prefix {path_prefix}"

        self.path_prefix = path_prefix                                         # trace_info : t_16259
        self.multimodal = multimodal                                           # trace_info : t_16260
        self.mmap = mmap                                                       # trace_info : t_16261

        self.index = _IndexReader(idx_path, self.multimodal)                   # trace_info : t_16262
        self.bin_buffer = None                                                 # trace_info : t_16373
        self.bin_buffer_mmap = None                                            # trace_info : t_16374
        if mmap:                                                               # trace_info : t_16375
            self.bin_buffer_mmap = numpy.memmap(bin_path, mode="r", order="C") # trace_info : t_16376
            self.bin_buffer = memoryview(self.bin_buffer_mmap)                 # trace_info : t_16377
    ...
    def __getstate__(self) -> Tuple[str, bool, bool]:
        ...
        """
        return self.path_prefix, self.multimodal, self.mmap                    # trace_info : t_17232, t_17233, t_17310, t_17311, t_17388, ...
    ...
    def __del__(self) -> None:
        """Clean up the object
        """
        if self.bin_buffer_mmap is not None:                                   # trace_info : t_156137
            self.bin_buffer_mmap._mmap.close()                                 # trace_info : t_156138
        del self.bin_buffer_mmap                                               # trace_info : t_156139
        del self.index                                                         # trace_info : t_156140
    ...
    def sequence_lengths(self) -> numpy.ndarray:
        ...
        """
        return self.index.sequence_lengths                                     # trace_info : t_16380
...
def get_idx_path(path_prefix: str) -> str:
    ...
    """
    return path_prefix + ".idx"                                                # trace_info : t_16253
...
def get_bin_path(path_prefix: str) -> str:
    ...
    """
    return path_prefix + ".bin"                                                # trace_info : t_16255
