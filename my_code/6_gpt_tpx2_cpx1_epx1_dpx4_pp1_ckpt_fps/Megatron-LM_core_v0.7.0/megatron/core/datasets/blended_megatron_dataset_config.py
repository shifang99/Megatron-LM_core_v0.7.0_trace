...
class BlendedMegatronDatasetConfig:
    ...
    def __post_init__(self) -> None:
        ...
        """
        if self.blend_per_split is not None and any(self.blend_per_split):     # trace_info : t_16108
            ...
            assert self.split is not None, "split must be provided in absence of blend_per_split"# trace_info : t_16109
            split_vector = parse_and_normalize_split(self.split)               # trace_info : t_16110
            self.split_matrix = convert_split_vector_to_split_matrix(split_vector)# trace_info : t_16124
            log_single_rank(logger, logging.INFO, f"Let split_matrix = {self.split_matrix}")# trace_info : t_16152
            if self.blend is not None:                                         # trace_info : t_16160
                assert self.blend[1] is None or len(self.blend[0]) == len(     # trace_info : t_16161
                    self.blend[1]
...
def parse_and_normalize_split(split: str) -> List[float]:
    ...
    """
    split = list(map(float, re.findall(r"[.0-9]+", split)))                    # trace_info : t_16111
    split = split + [0.0 for _ in range(len(Split) - len(split))]              # trace_info : t_16112

    assert len(split) == len(Split)                                            # trace_info : t_16113
    assert all(map(lambda _: _ >= 0.0, split))                                 # trace_info : t_16114, t_16115, t_16116, t_16117

    split = normalize(split)                                                   # trace_info : t_16118

    return split                                                               # trace_info : t_16123
...
def convert_split_vector_to_split_matrix(
    ...
    """
    if vector_b is None:                                                       # trace_info : t_16125
        vector_b = vector_a                                                    # trace_info : t_16126

    # [.900, .090, .010] -> [0.00, .900, .990, 100]
    expansion_a = functools.reduce(lambda a, b: a + [a[len(a) - 1] + b], [[0], *vector_a])# trace_info : t_16127, t_16128, t_16129, t_16130
    expansion_b = functools.reduce(lambda a, b: a + [a[len(a) - 1] + b], [[0], *vector_b])# trace_info : t_16131, t_16132, t_16133, t_16134

    # [0.00, .900, .990, 100.0] -> [(0.00, .900), (.900, .990), (.990, 100)]
    bookends_a = list(zip(expansion_a[:-1], expansion_a[1:]))                  # trace_info : t_16135
    bookends_b = list(zip(expansion_b[:-1], expansion_b[1:]))                  # trace_info : t_16136

    # gather per-split overlap or None
    matrix = []                                                                # trace_info : t_16137
    for bookend_a, bookend_b in zip(bookends_a, bookends_b):                   # trace_info : t_16138, t_16142, t_16146, t_16150
        if min(bookend_a[1], bookend_b[1]) <= max(bookend_a[0], bookend_b[0]): # trace_info : t_16139, t_16143, t_16147
            overlap = None
        else:
            overlap = (max(bookend_a[0], bookend_b[0]), min(bookend_a[1], bookend_b[1]))# trace_info : t_16140, t_16144, t_16148
        matrix.append(overlap)                                                 # trace_info : t_16141, t_16145, t_16149

    return matrix                                                              # trace_info : t_16151
