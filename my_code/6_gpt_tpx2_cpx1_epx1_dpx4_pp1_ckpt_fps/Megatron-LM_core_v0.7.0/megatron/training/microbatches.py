...
def build_num_microbatches_calculator(args):
    ...
    if args.rampup_batch_size is None:                                         # trace_info : t_3114
        num_microbatches_calculator = ConstantNumMicroBatches(                 # trace_info : t_3115, t_3118
            args.global_batch_size, args.micro_batch_size,                     # trace_info : t_3116
            args.data_parallel_size)                                           # trace_info : t_3117
        if args.rank == 0:                                                     # trace_info : t_3128
            print('setting number of micro-batches to constant {}'.format(     # trace_info : t_3129, t_3132, t_3134
                num_microbatches_calculator.get()), flush=True)                # trace_info : t_3130, t_3133
    ...
    return num_microbatches_calculator                                         # trace_info : t_3135
...
class NumMicroBatchesCalculator(ABC):
    ...
    def get(self):
        return self.num_micro_batches                                          # trace_info : t_3131, t_17656, t_17676, t_17679, t_17949, ...
...
class ConstantNumMicroBatches(NumMicroBatchesCalculator):
    ...
    def __init__(self, global_batch_size, micro_batch_size, data_parallel_size):
        micro_batch_times_data_parallel = micro_batch_size * \                 # trace_info : t_3119, t_3121
                                          data_parallel_size                   # trace_info : t_3120
        assert global_batch_size % micro_batch_times_data_parallel == 0, \     # trace_info : t_3122
            'global batch size ({}) is not divisible by micro batch size ({})' \
            ' times data parallel size ({})'.format(global_batch_size,
                                                    micro_batch_size,
                                                    data_parallel_size)
        self.num_micro_batches = global_batch_size // \                        # trace_info : t_3123, t_3125
                                 micro_batch_times_data_parallel               # trace_info : t_3124
        assert self.num_micro_batches >= 1                                     # trace_info : t_3126
        self.current_global_batch_size = global_batch_size                     # trace_info : t_3127

    def update(self, consumed_samples, consistency_check):
        pass                                                                   # trace_info : t_17673, t_17684, t_21256, t_21267, t_88863, ...
