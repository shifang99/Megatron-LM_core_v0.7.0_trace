...
def validate_args(args, defaults={}):
    ...
    if args.bf16:                                                              # trace_info : t_1154
    ...
    if args.rank == 0:                                                         # trace_info : t_1155
        print('using {} for parameters ...'.format(args.params_dtype),         # trace_info : t_1156, t_1158
              flush=True)                                                      # trace_info : t_1157

    if args.dataloader_type is None:                                           # trace_info : t_1159
        args.dataloader_type = 'single'                                        # trace_info : t_1160

    # data
    assert args.num_dataset_builder_threads > 0                                # trace_info : t_1161

    # Consumed tokens.
    args.consumed_train_samples = 0                                            # trace_info : t_1162
    args.consumed_valid_samples = 0                                            # trace_info : t_1163
    ...
    args.variable_seq_lengths = False                                          # trace_info : t_1164

    # Iteration-based training.
    if args.train_iters:                                                       # trace_info : t_1165
        # If we use iteration-based training, make sure the
        # sample-based options are off.
        assert args.train_samples is None, \                                   # trace_info : t_1166
            'expected iteration-based training'
        assert args.lr_decay_samples is None, \                                # trace_info : t_1167
            'expected iteration-based learning rate decay'
        assert args.lr_warmup_samples == 0, \                                  # trace_info : t_1168
            'expected iteration-based learning rate warmup'
        assert args.rampup_batch_size is None, \                               # trace_info : t_1169
            'expected no batch-size rampup for iteration-based training'
        if args.lr_warmup_fraction is not None:                                # trace_info : t_1170
            assert args.lr_warmup_iters == 0, \                                # trace_info : t_1171
                'can only specify one of lr-warmup-fraction and lr-warmup-iters'

    # Sample-based training.
    if args.train_samples:                                                     # trace_info : t_1172
    ...
    if args.num_layers is not None:                                            # trace_info : t_1173
        assert args.encoder_num_layers is None, \                              # trace_info : t_1174
            'cannot have both num-layers and encoder-num-layers specified'
        args.encoder_num_layers = args.num_layers                              # trace_info : t_1175
    else:
    ...
    required_args = ['num_layers', 'hidden_size', 'num_attention_heads',       # trace_info : t_1176
                     'max_position_embeddings']
    for req_arg in required_args:                                              # trace_info : t_1177, t_1180, t_1183, t_1186, t_1189
        _check_arg_is_not_none(args, req_arg)                                  # trace_info : t_1178, t_1181, t_1184, t_1187

    # Checks.
    if args.ffn_hidden_size is None:                                           # trace_info : t_1190
        if args.swiglu:                                                        # trace_info : t_1191
            ...
            args.ffn_hidden_size = 4 * args.hidden_size                        # trace_info : t_1192

    if args.kv_channels is None:                                               # trace_info : t_1193
        assert args.hidden_size % args.num_attention_heads == 0                # trace_info : t_1194
        args.kv_channels = args.hidden_size // args.num_attention_heads        # trace_info : t_1195

    if args.seq_length is not None:                                            # trace_info : t_1196
        assert args.encoder_seq_length is None                                 # trace_info : t_1197
        args.encoder_seq_length = args.seq_length                              # trace_info : t_1198
    else:
        assert args.encoder_seq_length is not None
        args.seq_length = args.encoder_seq_length

    if args.seq_length is not None:                                            # trace_info : t_1199
        assert args.max_position_embeddings >= args.seq_length                 # trace_info : t_1200
    if args.decoder_seq_length is not None:                                    # trace_info : t_1201
        assert args.max_position_embeddings >= args.decoder_seq_length
    if args.lr is not None:                                                    # trace_info : t_1202
        assert args.min_lr <= args.lr                                          # trace_info : t_1203
    if args.save is not None:                                                  # trace_info : t_1204
        assert args.save_interval is not None
    # Mixed precision checks.
    if args.fp16_lm_cross_entropy:                                             # trace_info : t_1205
        assert args.fp16, 'lm cross entropy in fp16 only support in fp16 mode.'
    if args.fp32_residual_connection:                                          # trace_info : t_1206
        assert args.fp16 or args.bf16, \
            'residual connection in fp32 only supported when using fp16 or bf16.'

    if args.moe_grouped_gemm:                                                  # trace_info : t_1207
        assert args.bf16, 'Currently GroupedGEMM for MoE only supports bf16 dtype.'
        dc = torch.cuda.get_device_capability()
        assert dc[0] >= 8, "Unsupported compute capability for GroupedGEMM kernels."

    if args.weight_decay_incr_style == 'constant':                             # trace_info : t_1208
        assert args.start_weight_decay is None                                 # trace_info : t_1209
        assert args.end_weight_decay is None                                   # trace_info : t_1210
        args.start_weight_decay = args.weight_decay                            # trace_info : t_1211
        args.end_weight_decay = args.weight_decay                              # trace_info : t_1212
    else:
        assert args.start_weight_decay is not None
        assert args.end_weight_decay is not None

    TORCH_MAJOR = int(torch.__version__.split('.')[0])                         # trace_info : t_1213
    TORCH_MINOR = int(torch.__version__.split('.')[1])                         # trace_info : t_1214
    # Persistent fused layer norm.
    if TORCH_MAJOR < 1 or (TORCH_MAJOR == 1 and TORCH_MINOR < 11):             # trace_info : t_1215
    ...
    if args.distribute_saved_activations:                                      # trace_info : t_1216
    ...
    if args.recompute_granularity == 'selective':                              # trace_info : t_1217
    ...
    if args.tensor_model_parallel_size == 1:                                   # trace_info : t_1218
        args.sequence_parallel = False

    # disable async_tensor_model_parallel_allreduce when
    # model parallel memory optimization is enabled
    if args.sequence_parallel:                                                 # trace_info : t_1219
        args.async_tensor_model_parallel_allreduce = False                     # trace_info : t_1220

    if os.environ.get('CUDA_DEVICE_MAX_CONNECTIONS') != "1":                   # trace_info : t_1221
    ...
    if not args.add_bias_linear:                                               # trace_info : t_1222
        args.bias_gelu_fusion = False

    # Retro checks.
    if args.retro_add_retriever:                                               # trace_info : t_1223
    ...
    if args.decoupled_lr is not None or args.decoupled_min_lr is not None:     # trace_info : t_1224
        assert args.use_mcore_models, \
            '--decoupled-lr and --decoupled-min-lr only supported by Megatron Core, please add --use-mcore-models.'

    # Legacy RoPE arguments
    if args.use_rotary_position_embeddings:                                    # trace_info : t_1225
        args.position_embedding_type = 'rope'
    if args.rotary_interleaved and args.apply_rope_fusion:                     # trace_info : t_1226
        raise RuntimeError('--rotary-interleaved does not work with rope_fusion.')
    if args.rotary_interleaved and not args.use_mcore_models:                  # trace_info : t_1227
        raise RuntimeError('--rotary-interleaved only support Megatron Core, please add --use-mcore-models.')

    # Would just need to add 'NoPE' as a position_embedding_type to support this, but for now
    # don't allow it to keep things simple
    if not args.add_position_embedding and args.position_embedding_type != 'rope':# trace_info : t_1228
        raise RuntimeError('--no-position-embedding is deprecated, use --position-embedding-type')

    # MoE Spec check
    if args.num_experts is not None:                                           # trace_info : t_1229
        assert args.spec is None, "Model Spec must be None when using MoEs"    # trace_info : t_1230

    # Expert parallelism check
    if args.expert_model_parallel_size  > 1:                                   # trace_info : t_1231
        assert args.num_experts is not None, "num_experts must be non None to use expert model parallelism"# trace_info : t_1232
        assert args.num_experts % args.expert_model_parallel_size == 0, \      # trace_info : t_1233
            "Number of experts should be a multiple of expert model parallel_size."
        assert not args.fp16, \                                                # trace_info : t_1234
            "Expert parallelism is not supported with fp16 training."

    # Distributed checkpointing checks
    if args.use_dist_ckpt and not args.use_mcore_models:                       # trace_info : t_1235
        raise RuntimeError('--use-dist-ckpt only support Megatron Core, please add --use-mcore-models.')

    # Data blend checks
    assert args.mock_data + \                                                  # trace_info : t_1236, t_1238, t_1240, t_1242
           bool(args.data_path) + \                                            # trace_info : t_1237
           any([args.train_data_path, args.valid_data_path, args.test_data_path]) \# trace_info : t_1239
           == 1, "A single data source must be provided"                       # trace_info : t_1241

    if args.use_tp_pp_dp_mapping:                                              # trace_info : t_1243
    ...
    # Print arguments.
    _print_args("arguments", args)                                             # trace_info : t_1244

    return args                                                                # trace_info : t_3097


def _print_args(title, args):
    """Print arguments."""
    if args.rank == 0:                                                         # trace_info : t_1245
        print(f'------------------------ {title} ------------------------',    # trace_info : t_1246, t_1248
              flush=True)                                                      # trace_info : t_1247
        str_list = []                                                          # trace_info : t_1249
        for arg in vars(args):                                                 # trace_info : t_1250, t_1253, t_1256, t_1259, t_1262, ...
            dots = '.' * (48 - len(arg))                                       # trace_info : t_1251, t_1254, t_1257, t_1260, t_1263, ...
            str_list.append('  {} {} {}'.format(arg, dots, getattr(args, arg)))# trace_info : t_1252, t_1255, t_1258, t_1261, t_1264, ...
        for arg in sorted(str_list, key=lambda x: x.lower()):                  # trace_info : t_2172, t_2173, t_2174, t_2175, t_2176, ...
            print(arg, flush=True)                                             # trace_info : t_2480, t_2482, t_2484, t_2486, t_2488, ...
        print(f'-------------------- end of {title} ---------------------',    # trace_info : t_3094, t_3096
              flush=True)                                                      # trace_info : t_3095


def _check_arg_is_not_none(args, arg):
    assert getattr(args, arg) is not None, '{} argument is None'.format(arg)   # trace_info : t_1179, t_1182, t_1185, t_1188


def core_transformer_config_from_args(args, config_class=None):

    # Config class.
    config_class = config_class or TransformerConfig                           # trace_info : t_8796

    # Translate args to core transformer configuration
    kw_args = {}                                                               # trace_info : t_8797
    for f in dataclasses.fields(config_class):                                 # trace_info : t_8798, t_8801, t_8804, t_8807, t_8810, ...
        if hasattr(args, f.name):                                              # trace_info : t_8799, t_8802, t_8805, t_8808, t_8811, ...
            kw_args[f.name] = getattr(args, f.name)                            # trace_info : t_8800, t_8803, t_8806, t_8809, t_8812, ...
    kw_args['persist_layer_norm'] = not args.no_persist_layer_norm             # trace_info : t_9102
    kw_args['layernorm_zero_centered_gamma'] = args.apply_layernorm_1p         # trace_info : t_9103
    kw_args['layernorm_epsilon'] = args.norm_epsilon                           # trace_info : t_9104
    kw_args['deallocate_pipeline_outputs'] = True                              # trace_info : t_9105
    kw_args['pipeline_dtype'] = args.params_dtype                              # trace_info : t_9106
    kw_args['batch_p2p_comm'] = not args.overlap_p2p_comm                      # trace_info : t_9107
    kw_args['num_moe_experts'] = args.num_experts                              # trace_info : t_9108
    kw_args['rotary_interleaved'] = args.rotary_interleaved                    # trace_info : t_9109
    if args.swiglu:                                                            # trace_info : t_9110
        kw_args['activation_func'] = F.silu
        kw_args['gated_linear_unit'] = True
        kw_args['bias_activation_fusion'] = args.bias_swiglu_fusion
    else:
        kw_args['bias_activation_fusion'] = args.bias_gelu_fusion              # trace_info : t_9111
    if args.squared_relu:                                                      # trace_info : t_9112
    ...
    if args.init_method_xavier_uniform:                                        # trace_info : t_9113
        kw_args['init_method'] = torch.nn.init.xavier_uniform_
        kw_args['scaled_init_method'] = torch.nn.init.xavier_uniform_
    if args.group_query_attention:                                             # trace_info : t_9114
        kw_args['num_query_groups'] = args.num_query_groups
    else:
        kw_args['num_query_groups'] = None                                     # trace_info : t_9115

    # Return config.
    return config_class(**kw_args)                                             # trace_info : t_9116
