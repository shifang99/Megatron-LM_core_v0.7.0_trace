...
class DType(Enum):
    ...
    def dtype_from_code(cls, value: int) -> Type[numpy.number]:
        ...
        """
        return getattr(numpy, cls(value).name)                                 # trace_info : t_16209
    ...
    def size(key: Union[int, Type[numpy.number]]) -> int:
        ...
        """
        if isinstance(key, int):                                               # trace_info : t_16211
            return DType.dtype_from_code(key)().itemsize
        elif numpy.number in key.__mro__:                                      # trace_info : t_16212
            return key().itemsize                                              # trace_info : t_16213
        else:
            raise ValueError
...
class _IndexReader(object):
    ...
    def __init__(self, idx_path: str, multimodal: bool) -> None:
        ...
        log_single_rank(logger, logging.INFO, f"Load the {type(self).__name__} from {idx_path}")# trace_info : t_16194

        with open(idx_path, "rb") as stream:                                   # trace_info : t_16202, t_16217
            header = stream.read(9)                                            # trace_info : t_16203
            assert header == _INDEX_HEADER, f"bad header, cannot read: {idx_path}"# trace_info : t_16204

            version = struct.unpack("<Q", stream.read(8))[0]                   # trace_info : t_16205
            assert version == 1, f"bad version, cannot read: {idx_path}"       # trace_info : t_16206

            code = struct.unpack("<B", stream.read(1))[0]                      # trace_info : t_16207
            self.dtype = DType.dtype_from_code(code)                           # trace_info : t_16208
            self.dtype_size = DType.size(self.dtype)                           # trace_info : t_16210

            self.sequence_count = struct.unpack("<Q", stream.read(8))[0]       # trace_info : t_16214
            self.document_count = struct.unpack("<Q", stream.read(8))[0]       # trace_info : t_16215

            offset = stream.tell()                                             # trace_info : t_16216

        self.bin_buffer_mmap = numpy.memmap(idx_path, mode="r", order="C")     # trace_info : t_16218
        self.bin_buffer = memoryview(self.bin_buffer_mmap)                     # trace_info : t_16219

        log_single_rank(logger, logging.INFO, f"\tExtract the sequence lengths")# trace_info : t_16220
        t_beg = time.time()                                                    # trace_info : t_16228
        self.sequence_lengths = numpy.frombuffer(                              # trace_info : t_16229, t_16231
            self.bin_buffer, dtype=numpy.int32, count=self.sequence_count, offset=offset# trace_info : t_16230
        )
        t_end = time.time()                                                    # trace_info : t_16232
        log_single_rank(logger, logging.DEBUG, f"\t> time elapsed: {t_end - t_beg:4f} seconds")# trace_info : t_16233

        log_single_rank(logger, logging.INFO, f"\tExtract the sequence pointers")# trace_info : t_16237
        t_beg = time.time()                                                    # trace_info : t_16245
        self.sequence_pointers = numpy.frombuffer(                             # trace_info : t_16246, t_16251
            self.bin_buffer,                                                   # trace_info : t_16247
            dtype=numpy.int64,                                                 # trace_info : t_16248
            count=self.sequence_count,                                         # trace_info : t_16249
            offset=offset + self.sequence_lengths.nbytes,                      # trace_info : t_16250
        )
        t_end = time.time()                                                    # trace_info : t_16252
        log_single_rank(logger, logging.DEBUG, f"\t> time elapsed: {t_end - t_beg:4f} seconds")# trace_info : t_16253

        log_single_rank(logger, logging.INFO, f"\tExtract the document indices")# trace_info : t_16257
        t_beg = time.time()                                                    # trace_info : t_16265
        self.document_indices = numpy.frombuffer(                              # trace_info : t_16266, t_16271
            self.bin_buffer,                                                   # trace_info : t_16267
            dtype=numpy.int64,                                                 # trace_info : t_16268
            count=self.document_count,                                         # trace_info : t_16269
            offset=offset + self.sequence_lengths.nbytes + self.sequence_pointers.nbytes,# trace_info : t_16270
        )
        t_end = time.time()                                                    # trace_info : t_16272
        log_single_rank(logger, logging.DEBUG, f"\t> time elapsed: {t_end - t_beg:4f} seconds")# trace_info : t_16273

        self.sequence_modes = None                                             # trace_info : t_16277
        if multimodal:                                                         # trace_info : t_16278
        ...
        assert self.sequence_lengths.shape[0] == len(self)                     # trace_info : t_16279
        assert self.sequence_lengths.shape[0] == self.sequence_count           # trace_info : t_16281
        assert self.sequence_lengths.shape[0] == self.document_indices[-1]     # trace_info : t_16282

        log_single_rank(logger, logging.INFO, f"> total number of sequences: {len(self)}")# trace_info : t_16283
        log_single_rank(                                                       # trace_info : t_16292, t_16296
            logger,                                                            # trace_info : t_16293
            logging.INFO,                                                      # trace_info : t_16294
            f"> total number of documents: {self.document_indices.shape[0] - 1}",# trace_info : t_16295
        )

    def __del__(self) -> None:
        """Clean up the object
        """
        if hasattr(self, "bin_buffer_mmap"):                                   # trace_info : t_30614
            self.bin_buffer_mmap._mmap.close()                                 # trace_info : t_30615
            del self.bin_buffer_mmap                                           # trace_info : t_30616
    ...
    def __len__(self) -> int:
        ...
        """
        return self.sequence_count                                             # trace_info : t_16280, t_16284
...
class IndexedDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, path_prefix: str, multimodal: bool = False, mmap: bool = True) -> None:
        super().__init__()                                                     # trace_info : t_16178
        self.path_prefix = None                                                # trace_info : t_16179
        self.multimodal = None                                                 # trace_info : t_16180
        self.mmap = None                                                       # trace_info : t_16181

        self.initialize(path_prefix, multimodal, mmap)                         # trace_info : t_16182
    ...
    def initialize(self, path_prefix: str, multimodal: bool, mmap: bool) -> None:
        ...
        """
        idx_path = get_idx_path(path_prefix)                                   # trace_info : t_16183
        bin_path = get_bin_path(path_prefix)                                   # trace_info : t_16185
        assert os.path.exists(idx_path) and os.path.exists(                    # trace_info : t_16187, t_16189
            bin_path                                                           # trace_info : t_16188
        ), f"One or both of the .idx and .bin files cannot be found at the path prefix {path_prefix}"

        self.path_prefix = path_prefix                                         # trace_info : t_16190
        self.multimodal = multimodal                                           # trace_info : t_16191
        self.mmap = mmap                                                       # trace_info : t_16192

        self.index = _IndexReader(idx_path, self.multimodal)                   # trace_info : t_16193
        self.bin_buffer = None                                                 # trace_info : t_16304
        self.bin_buffer_mmap = None                                            # trace_info : t_16305
        if mmap:                                                               # trace_info : t_16306
            self.bin_buffer_mmap = numpy.memmap(bin_path, mode="r", order="C") # trace_info : t_16307
            self.bin_buffer = memoryview(self.bin_buffer_mmap)                 # trace_info : t_16308
    ...
    def __getstate__(self) -> Tuple[str, bool, bool]:
        ...
        """
        return self.path_prefix, self.multimodal, self.mmap                    # trace_info : t_17163, t_17164, t_17241, t_17242, t_17319, ...
    ...
    def __del__(self) -> None:
        """Clean up the object
        """
        if self.bin_buffer_mmap is not None:                                   # trace_info : t_30610
            self.bin_buffer_mmap._mmap.close()                                 # trace_info : t_30611
        del self.bin_buffer_mmap                                               # trace_info : t_30612
        del self.index                                                         # trace_info : t_30613
    ...
    def sequence_lengths(self) -> numpy.ndarray:
        ...
        """
        return self.index.sequence_lengths                                     # trace_info : t_16311
...
def get_idx_path(path_prefix: str) -> str:
    ...
    """
    return path_prefix + ".idx"                                                # trace_info : t_16184
...
def get_bin_path(path_prefix: str) -> str:
    ...
    """
    return path_prefix + ".bin"                                                # trace_info : t_16186
