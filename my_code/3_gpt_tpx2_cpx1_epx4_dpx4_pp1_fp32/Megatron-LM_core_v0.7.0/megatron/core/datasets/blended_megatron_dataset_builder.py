...
class BlendedMegatronDatasetBuilder(object):
    ...
    def __init__(
        ...
        self.cls = cls                                                         # trace_info : t_16109
        self.sizes = sizes                                                     # trace_info : t_16110
        self.is_built_on_rank = is_built_on_rank                               # trace_info : t_16111
        self.config = config                                                   # trace_info : t_16112

        log_single_rank(                                                       # trace_info : t_16113, t_16118
            logger,                                                            # trace_info : t_16114
            logging.INFO,                                                      # trace_info : t_16115
            f"Building dataset splits with cls={cls.__name__}, sizes={self.sizes}, and config={self.config}",# trace_info : t_16116
        )

        if not self.config.mock:                                               # trace_info : t_16126
            for split in Split:                                                # trace_info : t_16127, t_16132, t_16137, t_16142
                size_is_none = self.sizes[split.value] is None                 # trace_info : t_16128, t_16133, t_16138
                if self.config.blend_per_split is None:                        # trace_info : t_16129, t_16134, t_16139
                    weights_are_none = self.config.blend[1] is None
                else:
                    if self.config.blend_per_split[split.value] is None:       # trace_info : t_16130, t_16135, t_16140
                        continue                                               # trace_info : t_16131, t_16136, t_16141
        ...
        if torch.distributed.is_initialized():                                 # trace_info : t_16143
            gb_rank = torch.distributed.get_rank()                             # trace_info : t_16144
            vp_rank = get_virtual_pipeline_model_parallel_rank()               # trace_info : t_16145
            if gb_rank == 0 and (vp_rank == 0 or vp_rank is None):             # trace_info : t_16147
                assert (                                                       # trace_info : t_16166
                    self.is_built_on_rank()                                    # trace_info : t_16148
    ...
    def build(self) -> List[Optional[TopLevelDataset]]:
        ...
        """
        datasets = self._build_blended_dataset_splits()                        # trace_info : t_16168

        for dataset in datasets:                                               # trace_info : t_16977, t_16981, t_16985, t_16989
            if dataset is not None and len(dataset) > 0:                       # trace_info : t_16978, t_16982, t_16986
                if isinstance(dataset, BlendedDataset):                        # trace_info : t_16980, t_16984, t_16988
                    # Check blend size
        ...
        return datasets                                                        # trace_info : t_16990
    ...
    def _build_blended_dataset_splits(self,) -> List[Optional[TopLevelDataset]]:
        ...
        ##
        if self.config.mock:                                                   # trace_info : t_16169
        ...
        ##
        elif self.config.blend:                                                # trace_info : t_16170
            prefixes, weights = self.config.blend                              # trace_info : t_16171
            if weights is not None:                                            # trace_info : t_16172
                weights = normalize(weights)

            split = self.config.split_matrix                                   # trace_info : t_16173

            # Blend consists of a single prefix
            if len(prefixes) == 1 and weights is None:                         # trace_info : t_16174
                return self._build_megatron_dataset_splits(prefixes[0], split, self.sizes)# trace_info : t_16175
    ...
    def _build_megatron_dataset_splits(
        ...
        low_level_dataset = self.cls.build_low_level_dataset(dataset_path, self.config)# trace_info : t_16176

        # Build the split indices for the low level dataset
        num_elements = self.cls.numel_low_level_dataset(low_level_dataset)     # trace_info : t_16309
        split_indices = []                                                     # trace_info : t_16312
        for i, _ in enumerate(Split):                                          # trace_info : t_16313, t_16318, t_16323, t_16328
            if split[i] is not None:                                           # trace_info : t_16314, t_16319, t_16324
                beg = int(round(split[i][0] * float(num_elements)))            # trace_info : t_16315, t_16320, t_16325
                end = int(round(split[i][1] * float(num_elements)))            # trace_info : t_16316, t_16321, t_16326
                split_indices.append(numpy.arange(start=beg, stop=end, step=1, dtype=numpy.int32))# trace_info : t_16317, t_16322, t_16327
            else:
                split_indices.append(None)

        # Build the mid level dataset
        mid_level_datasets = []                                                # trace_info : t_16329
        for i, _split in enumerate(Split):                                     # trace_info : t_16330, t_16545, t_16760, t_16975
            if split[i] is None:                                               # trace_info : t_16331, t_16546, t_16761
                mid_level_datasets.append(None)
            else:
                mid_level_datasets.append(                                     # trace_info : t_16332, t_16544, t_16547, t_16759, t_16762, ...
                    self.build_generic_dataset(                                # trace_info : t_16333, t_16343, t_16548, t_16558, t_16763, ...
                        self.cls,                                              # trace_info : t_16334, t_16549, t_16764
                        self.is_built_on_rank,                                 # trace_info : t_16335, t_16550, t_16765
                        synchronize_ranks,                                     # trace_info : t_16336, t_16551, t_16766
                        low_level_dataset,                                     # trace_info : t_16337, t_16552, t_16767
                        dataset_path,                                          # trace_info : t_16338, t_16553, t_16768
                        split_indices[i],                                      # trace_info : t_16339, t_16554, t_16769
                        sizes[i],                                              # trace_info : t_16340, t_16555, t_16770
                        _split,                                                # trace_info : t_16341, t_16556, t_16771
                        self.config,                                           # trace_info : t_16342, t_16557, t_16772
                    )
                )

        return mid_level_datasets                                              # trace_info : t_16976
    ...
    def build_generic_dataset(
        ...
        *args: Any,
        ...
        """
        if torch.distributed.is_initialized():                                 # trace_info : t_16344, t_16559, t_16774
            rank = torch.distributed.get_rank()                                # trace_info : t_16345, t_16560, t_16775

            dataset = None                                                     # trace_info : t_16346, t_16561, t_16776

            # First, build on rank 0
            if rank == 0 and is_built_on_rank():                               # trace_info : t_16347, t_16562, t_16777
                try:                                                           # trace_info : t_16365, t_16580, t_16795
                    dataset = cls(*args)                                       # trace_info : t_16366, t_16581, t_16796
            ...
            if synchronize_ranks:                                              # trace_info : t_16540, t_16755, t_16970
                torch.distributed.barrier()                                    # trace_info : t_16541, t_16756, t_16971

            # After, build on other ranks
            if rank != 0 and is_built_on_rank():                               # trace_info : t_16542, t_16757, t_16972
                dataset = cls(*args)

            return dataset                                                     # trace_info : t_16543, t_16758, t_16973
