...
class BlendedMegatronDatasetConfig:
    ...
    def __post_init__(self) -> None:
        ...
        """
        if self.blend_per_split is not None and any(self.blend_per_split):     # trace_info : t_16039
            ...
            assert self.split is not None, "split must be provided in absence of blend_per_split"# trace_info : t_16040
            split_vector = parse_and_normalize_split(self.split)               # trace_info : t_16041
            self.split_matrix = convert_split_vector_to_split_matrix(split_vector)# trace_info : t_16055
            log_single_rank(logger, logging.INFO, f"Let split_matrix = {self.split_matrix}")# trace_info : t_16083
            if self.blend is not None:                                         # trace_info : t_16091
                assert self.blend[1] is None or len(self.blend[0]) == len(     # trace_info : t_16092
                    self.blend[1]
...
def parse_and_normalize_split(split: str) -> List[float]:
    ...
    """
    split = list(map(float, re.findall(r"[.0-9]+", split)))                    # trace_info : t_16042
    split = split + [0.0 for _ in range(len(Split) - len(split))]              # trace_info : t_16043

    assert len(split) == len(Split)                                            # trace_info : t_16044
    assert all(map(lambda _: _ >= 0.0, split))                                 # trace_info : t_16045, t_16046, t_16047, t_16048

    split = normalize(split)                                                   # trace_info : t_16049

    return split                                                               # trace_info : t_16054
...
def convert_split_vector_to_split_matrix(
    ...
    """
    if vector_b is None:                                                       # trace_info : t_16056
        vector_b = vector_a                                                    # trace_info : t_16057

    # [.900, .090, .010] -> [0.00, .900, .990, 100]
    expansion_a = functools.reduce(lambda a, b: a + [a[len(a) - 1] + b], [[0], *vector_a])# trace_info : t_16058, t_16059, t_16060, t_16061
    expansion_b = functools.reduce(lambda a, b: a + [a[len(a) - 1] + b], [[0], *vector_b])# trace_info : t_16062, t_16063, t_16064, t_16065

    # [0.00, .900, .990, 100.0] -> [(0.00, .900), (.900, .990), (.990, 100)]
    bookends_a = list(zip(expansion_a[:-1], expansion_a[1:]))                  # trace_info : t_16066
    bookends_b = list(zip(expansion_b[:-1], expansion_b[1:]))                  # trace_info : t_16067

    # gather per-split overlap or None
    matrix = []                                                                # trace_info : t_16068
    for bookend_a, bookend_b in zip(bookends_a, bookends_b):                   # trace_info : t_16069, t_16073, t_16077, t_16081
        if min(bookend_a[1], bookend_b[1]) <= max(bookend_a[0], bookend_b[0]): # trace_info : t_16070, t_16074, t_16078
            overlap = None
        else:
            overlap = (max(bookend_a[0], bookend_b[0]), min(bookend_a[1], bookend_b[1]))# trace_info : t_16071, t_16075, t_16079
        matrix.append(overlap)                                                 # trace_info : t_16072, t_16076, t_16080

    return matrix                                                              # trace_info : t_16082
