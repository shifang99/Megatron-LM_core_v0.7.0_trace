...
def ensure_divisibility(numerator, denominator):
    ...
    assert numerator % denominator == 0, "{} is not divisible by {}".format(numerator, denominator)# trace_info : t_9414, t_9683, t_9687, t_9691, t_9732, ...


def divide(numerator, denominator):
    """Ensure that numerator is divisible by the denominator and return
    the division value."""
    ensure_divisibility(numerator, denominator)                                # trace_info : t_9413, t_9682, t_9686, t_9690, t_9731, ...
    return numerator // denominator                                            # trace_info : t_9415, t_9684, t_9688, t_9692, t_9733, ...
...
def get_attr_wrapped_model(model, attr, allow_none=True, return_model_obj=False):
    ...
    if isinstance(model, list):                                                # trace_info : t_12406, t_15894, t_17854, t_17874, t_18028, ...
        raise RuntimeError("_get_attr_wrapped_model given a list of models")

    if allow_none:                                                             # trace_info : t_12407, t_15895, t_17855, t_17875, t_18029, ...

        def condition(model, attr):                                            # trace_info : t_17876, t_18030, t_18048, t_20081, t_20271, ...
            return not hasattr(model, attr)                                    # trace_info : t_17878, t_17882, t_18032, t_18036, t_18050, ...

    else:

        def condition(model, attr):                                            # trace_info : t_12408, t_15896, t_17856, t_20177, t_22209, ...
            return getattr(model, attr, None) is None                          # trace_info : t_12410, t_15898, t_17858, t_20179, t_22211, ...

    while condition(model, attr):                                              # trace_info : t_12409, t_15897, t_17857, t_17877, t_17881, ...
        if not hasattr(model, "module"):                                       # trace_info : t_17879, t_18033, t_18051, t_20084, t_22232, ...
            raise RuntimeError(f"_get_attr_wrapped_model couldn't find attribute {attr}")

        model = model.module                                                   # trace_info : t_17880, t_18034, t_18052, t_20085, t_22233, ...

    if return_model_obj:                                                       # trace_info : t_12411, t_15899, t_17859, t_17883, t_18037, ...
        return model
    return getattr(model, attr)                                                # trace_info : t_12412, t_15900, t_17860, t_17884, t_18038, ...


def get_model_type(model):
    return get_attr_wrapped_model(model, 'model_type')                         # trace_info : t_17873, t_20078, t_22226, t_24423, t_26571, ...


def get_model_config(model):
    return get_attr_wrapped_model(model, 'config', allow_none=False)           # trace_info : t_12405, t_15893, t_17853, t_20174, t_22206, ...
...
class GlobalMemoryBuffer:
    ...
    def __init__(self):
        self.buffer = {}                                                       # trace_info : t_8316

    def get_tensor(self, tensor_shape, dtype, name):
        required_len = reduce(operator.mul, tensor_shape, 1)                   # trace_info : t_18428, t_18506, t_18820, t_19191, t_19267, ...
        if (
            self.buffer.get((name, dtype), None) is None                       # trace_info : t_18429, t_18507, t_18821, t_19192, t_19268, ...
            or self.buffer[(name, dtype)].numel() < required_len               # trace_info : t_18508, t_18822, t_19193, t_19269, t_19577, ...
        ):
            self.buffer[(name, dtype)] = torch.empty(                          # trace_info : t_18430, t_18432, t_18509, t_18511
                required_len, dtype=dtype, device=torch.cuda.current_device(), requires_grad=False# trace_info : t_18431, t_18510
            )

        return self.buffer[(name, dtype)][0:required_len].view(*tensor_shape)  # trace_info : t_18433, t_18512, t_18823, t_19194, t_19270, ...
...
def _kernel_make_viewless_tensor(inp, requires_grad):
    ...
    field.
    '''
    out = torch.empty((1,), dtype=inp.dtype, device=inp.device, requires_grad=requires_grad,)# trace_info : t_18358, t_18667, t_19121, t_19425, t_22711, ...
    out.data = inp.data                                                        # trace_info : t_18359, t_18668, t_19122, t_19426, t_22712, ...
    return out                                                                 # trace_info : t_18360, t_18669, t_19123, t_19427, t_22713, ...
...
class MakeViewlessTensor(torch.autograd.Function):
    ...
    def forward(ctx, inp, requires_grad):
        return _kernel_make_viewless_tensor(inp, requires_grad)                # trace_info : t_18357, t_18666, t_19120, t_19424, t_22710, ...
...
def make_viewless_tensor(inp, requires_grad, keep_graph):
    ...
    if inp._base is None:                                                      # trace_info : t_18319, t_18354, t_18663, t_19085, t_19117, ...
        return inp                                                             # trace_info : t_18320, t_19086, t_19841, t_22673, t_23431, ...

    # create viewless tensor
    if keep_graph:                                                             # trace_info : t_18355, t_18664, t_19118, t_19422, t_22708, ...
        return MakeViewlessTensor.apply(inp, requires_grad)                    # trace_info : t_18356, t_18665, t_19119, t_19423, t_22709, ...
    else:
...
def init_method_normal(sigma):
    ...
    def init_(tensor):                                                         # trace_info : t_9270
        return torch.nn.init.normal_(tensor, mean=0.0, std=sigma)              # trace_info : t_9472, t_9492, t_10023, t_10319, t_10467, ...

    return init_                                                               # trace_info : t_9271


def scaled_init_method_normal(sigma, num_layers):
    """Init method based on N(0, sigma/sqrt(2*num_layers)."""
    std = sigma / math.sqrt(2.0 * num_layers)                                  # trace_info : t_9276

    def init_(tensor):                                                         # trace_info : t_9277
        return torch.nn.init.normal_(tensor, mean=0.0, std=std)                # trace_info : t_9881, t_10628, t_11021, t_11768

    return init_                                                               # trace_info : t_9278
...
class StragglerDetector:
    ...
    def null_method(self) -> None:
        ...
        pass                                                                   # trace_info : t_18086, t_18193, t_18205, t_20009, t_22439, ...
    ...
    def __enter__(self) -> "StragglerDetector":
        ...
        """
        self.start()                                                           # trace_info : t_18085, t_18204, t_22438, t_22557, t_26783, ...
        return self                                                            # trace_info : t_18087, t_18206, t_22440, t_22559, t_26785, ...
    ...
    def __call__(self, bdata: bool = False) -> "StragglerDetector":
        ...
        Returns:
            StragglerDetector: the instance
        """
        self.bdata = bdata                                                     # trace_info : t_18083, t_22436, t_26781
        return self                                                            # trace_info : t_18084, t_22437, t_26782
    ...
    def __exit__(
        ...
        ret = False                                                            # trace_info : t_18190, t_20006, t_22543, t_24351, t_26888, ...
        if ex_type is not None:                                                # trace_info : t_18191, t_20007, t_22544, t_24352, t_26889, ...
            err = traceback.format_exception(ex_tb)
            logger.warning(f"{str(ex_val)}\n{err}")
            ret = True
        self.stop()                                                            # trace_info : t_18192, t_20008, t_22545, t_24353, t_26890, ...
        return ret                                                             # trace_info : t_18194, t_20010, t_22547, t_24355, t_26892, ...
