...
def switch_load_balancing_loss_func(
    ...
    """
    num_tokens = probs.shape[0] * topk                                         # trace_info : t_18731, t_19489, t_23079, t_23834, t_27424, ...
    num_experts = probs.shape[1]                                               # trace_info : t_18732, t_19490, t_23080, t_23835, t_27425, ...

    probs_mean_per_expert = probs.mean(dim=0)                                  # trace_info : t_18733, t_19491, t_23081, t_23836, t_27426, ...
    aux_loss = torch.sum(probs_mean_per_expert * tokens_per_expert) * (        # trace_info : t_18734, t_18736, t_19492, t_19494, t_23082, ...
        num_experts / num_tokens * moe_aux_loss_coeff                          # trace_info : t_18735, t_19493, t_23083, t_23838, t_27428, ...
    )
    return aux_loss                                                            # trace_info : t_18737, t_19495, t_23085, t_23840, t_27430, ...
...
class MoEAuxLossAutoScaler(torch.autograd.Function):
    ...
    def forward(ctx, output: torch.Tensor, aux_loss: torch.Tensor):
        ...
        """
        ctx.save_for_backward(aux_loss)                                        # trace_info : t_18751, t_19506, t_23096, t_23851, t_27441, ...
        return output                                                          # trace_info : t_18752, t_19507, t_23097, t_23852, t_27442, ...
    ...
    def set_loss_scale(scale: torch.Tensor):
        ...
        """
        MoEAuxLossAutoScaler.main_loss_backward_scale = scale                  # trace_info : t_20076, t_24421, t_28766
...
def topk_softmax_with_capacity(
    ...
    assert logits.dim() == 2, f"Expected 2D logits [num_tokens, num_experts], got {logits.dim()}."# trace_info : t_18711, t_19469, t_23059, t_23814, t_27404, ...
    num_tokens = logits.shape[0]                                               # trace_info : t_18712, t_19470, t_23060, t_23815, t_27405, ...
    num_experts = logits.shape[1]                                              # trace_info : t_18713, t_19471, t_23061, t_23816, t_27406, ...

    scores, top_indices = torch.topk(logits, k=topk, dim=1)                    # trace_info : t_18714, t_19472, t_23062, t_23817, t_27407, ...
    probs = torch.softmax(scores, dim=-1, dtype=torch.float32).type_as(logits) # trace_info : t_18715, t_19473, t_23063, t_23818, t_27408, ...

    if capacity_factor is None:                                                # trace_info : t_18716, t_19474, t_23064, t_23819, t_27409, ...
        # TopK without capacity
        tokens_per_expert = torch.histc(top_indices, bins=num_experts, min=0, max=num_experts)# trace_info : t_18717, t_19475, t_23065, t_23820, t_27410, ...
        return probs, top_indices, tokens_per_expert                           # trace_info : t_18718, t_19476, t_23066, t_23821, t_27411, ...
    else:
        # TopK with capacity
...
def save_to_aux_losses_tracker(name: str, loss: torch.Tensor, layer_number: int, num_layers: int):
    ...
    if layer_number is None:                                                   # trace_info : t_18744, t_19502, t_23092, t_23847, t_27437, ...
        return

    if name not in parallel_state._MOE_AUX_LOSSES_LOGGING_TRACKER:             # trace_info : t_18745, t_19503, t_23093, t_23848, t_27438, ...
        parallel_state._MOE_AUX_LOSSES_LOGGING_TRACKER[name] = torch.zeros(    # trace_info : t_18746, t_18748
            num_layers, device=loss.device                                     # trace_info : t_18747
        )
    parallel_state._MOE_AUX_LOSSES_LOGGING_TRACKER[name][layer_number - 1] += loss.detach()# trace_info : t_18749, t_19504, t_23094, t_23849, t_27439, ...


def clear_aux_losses_tracker():
    """Clear the auxiliary losses."""
    for name in parallel_state._MOE_AUX_LOSSES_LOGGING_TRACKER:                # trace_info : t_21868, t_21870, t_26213, t_26215, t_30558, ...
        parallel_state._MOE_AUX_LOSSES_LOGGING_TRACKER[name].zero_()           # trace_info : t_21869, t_26214, t_30559
...
def aggregate_aux_losses_tracker_across_pipeline_parallel():
    ...
    for name in parallel_state._MOE_AUX_LOSSES_LOGGING_TRACKER:                # trace_info : t_21860, t_21865, t_26205, t_26210, t_30550, ...
        loss = parallel_state._MOE_AUX_LOSSES_LOGGING_TRACKER[name]            # trace_info : t_21861, t_26206, t_30551
        torch.distributed.all_reduce(loss, group=parallel_state.get_pipeline_model_parallel_group())# trace_info : t_21862, t_26207, t_30552
...
def track_moe_metrics(
    ...
    # Aux loss logging
    aggregate_aux_losses_tracker_across_pipeline_parallel()                    # trace_info : t_21859, t_26204, t_30549
    if writer is not None:                                                     # trace_info : t_21866, t_26211, t_30556
    ...
    clear_aux_losses_tracker()                                                 # trace_info : t_21867, t_26212, t_30557


class moe_gather(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input_, map_):
        ctx.input_size = input_.size()                                         # trace_info : t_18829, t_19584, t_23174, t_23929, t_27519, ...
        ctx.map = map_                                                         # trace_info : t_18830, t_19585, t_23175, t_23930, t_27520, ...
        return torch.gather(input_, 0, map_)                                   # trace_info : t_18831, t_19586, t_23176, t_23931, t_27521, ...
...
class moe_scatter(torch.autograd.Function):
    ...
    def forward(ctx, input_, map_, output_size=None):
        ctx.map = map_                                                         # trace_info : t_19009, t_19764, t_23354, t_24109, t_27699, ...

        if output_size is not None:                                            # trace_info : t_19010, t_19765, t_23355, t_24110, t_27700, ...
            output = torch.zeros(                                              # trace_info : t_19011, t_19013, t_19766, t_19768, t_23356, ...
                output_size, dtype=input_.dtype, device=torch.cuda.current_device()# trace_info : t_19012, t_19767, t_23357, t_24112, t_27702, ...
            )
        else:
            output = torch.zeros_like(input_)

        output.scatter_add_(0, map_, input_)                                   # trace_info : t_19014, t_19769, t_23359, t_24114, t_27704, ...
        return output                                                          # trace_info : t_19015, t_19770, t_23360, t_24115, t_27705, ...
