...
class LanguageModule(MegatronModule):
    ...
    def __init__(self, config: TransformerConfig) -> None:
        super().__init__(config=config)                                        # trace_info : t_9359
    ...
    def compute_language_model_loss(self, labels: Tensor, logits: Tensor) -> Tensor:
        ...
        """
        # [b s] => [s b]
        labels = labels.transpose(0, 1).contiguous()                           # trace_info : t_19937, t_24282, t_28627
        loss = tensor_parallel.vocab_parallel_cross_entropy(logits.float(), labels)# trace_info : t_19938, t_24283, t_28628

        # [s b] => [b, s]
        loss = loss.transpose(0, 1).contiguous()                               # trace_info : t_20002, t_24347, t_28692
        return loss                                                            # trace_info : t_20003, t_24348, t_28693
    ...
    def setup_embeddings_and_output_layer(self) -> None:
        ...
        if self.pre_process:                                                   # trace_info : t_11915
            self.embedding.word_embeddings.weight.is_embedding_or_output_parameter = True# trace_info : t_11916
        if self.post_process and self.output_layer.weight is not None:         # trace_info : t_11917
            self.output_layer.weight.is_embedding_or_output_parameter = True

        if not self.share_embeddings_and_output_weights:                       # trace_info : t_11918
            return

        if self.pre_process and self.post_process:                             # trace_info : t_11919
            # Zero out wgrad if sharing embeddings between two layers on same
            # pipeline stage to make sure grad accumulation into main_grad is
            # correct and does not include garbage values (e.g., from torch.empty).
            self.shared_embedding_or_output_weight().zero_out_wgrad = True     # trace_info : t_11920
            return                                                             # trace_info : t_11923
    ...
    def shared_embedding_or_output_weight(self) -> Tensor:
        ...
        """
        if self.pre_process:                                                   # trace_info : t_11921, t_19858, t_24203, t_28548
            return self.embedding.word_embeddings.weight                       # trace_info : t_11922, t_19859, t_24204, t_28549
