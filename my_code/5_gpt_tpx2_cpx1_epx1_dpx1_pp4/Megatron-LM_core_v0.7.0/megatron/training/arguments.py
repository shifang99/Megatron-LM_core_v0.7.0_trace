...
def validate_args(args, defaults={}):
    ...
    if args.pipeline_model_parallel_size > 1:                                  # trace_info : t_1126
        if args.pipeline_model_parallel_split_rank is not None:                # trace_info : t_1127
            assert args.pipeline_model_parallel_split_rank < \
                    args.pipeline_model_parallel_size, 'split rank needs'\
                    ' to be less than pipeline model parallel size ({})'.format(
                            args.pipeline_model_parallel_size)

    if args.tp_comm_overlap:                                                   # trace_info : t_1128
        assert args.sequence_parallel == True, 'Tensor parallel communication/GEMM overlap can happen only when sequence parallelism is enabled'

    # Deprecated arguments
    assert args.batch_size is None, '--batch-size argument is no longer ' \    # trace_info : t_1129
        'valid, use --micro-batch-size instead'
    del args.batch_size                                                        # trace_info : t_1130
    assert args.warmup is None, '--warmup argument is no longer valid, use ' \ # trace_info : t_1131
        '--lr-warmup-fraction instead'
    del args.warmup                                                            # trace_info : t_1132
    assert args.model_parallel_size is None, '--model-parallel-size is no ' \  # trace_info : t_1133
        'longer valid, use --tensor-model-parallel-size instead'
    del args.model_parallel_size                                               # trace_info : t_1134

    if args.checkpoint_activations:                                            # trace_info : t_1135
        if args.rank == 0:
            print('--checkpoint-activations is no longer valid, use --recompute-activations, '
                  'or, for more control, --recompute-granularity and --recompute-method.')
        exit()
    del args.checkpoint_activations                                            # trace_info : t_1136

    if args.recompute_activations:                                             # trace_info : t_1137
        args.recompute_granularity = 'selective'
    del args.recompute_activations                                             # trace_info : t_1138

    # Set input defaults.
    for key in defaults:                                                       # trace_info : t_1139, t_1142
        # For default to be valid, it should not be provided in the
        # arguments that are passed to the program. We check this by
        # ensuring the arg is set to None.
        if getattr(args, key, None) is not None:                               # trace_info : t_1140
            if args.rank == 0:
            ...
            setattr(args, key, defaults[key])                                  # trace_info : t_1141

    # Batch size.
    assert args.micro_batch_size is not None                                   # trace_info : t_1143
    assert args.micro_batch_size > 0                                           # trace_info : t_1144
    if args.global_batch_size is None:                                         # trace_info : t_1145
        args.global_batch_size = args.micro_batch_size * args.data_parallel_size
        if args.rank == 0:
            print('setting global batch size to {}'.format(
                args.global_batch_size), flush=True)
    assert args.global_batch_size > 0                                          # trace_info : t_1146
    if args.num_layers_per_virtual_pipeline_stage is not None:                 # trace_info : t_1147
        ...
        args.virtual_pipeline_model_parallel_size = None                       # trace_info : t_1148
        # Overlap P2P communication is disabled if not using the interleaved schedule.
        args.overlap_p2p_comm = False                                          # trace_info : t_1149
        if args.rank == 0:                                                     # trace_info : t_1150
            print('WARNING: Setting args.overlap_p2p_comm to False since non-interleaved '# trace_info : t_1151
                  'schedule does not support overlapping p2p communication')

    if args.overlap_param_gather:                                              # trace_info : t_1152
    ...
    # Parameters dtype.
    args.params_dtype = torch.float                                            # trace_info : t_1153
    if args.fp16:                                                              # trace_info : t_1154
        assert not args.bf16                                                   # trace_info : t_1155
        args.params_dtype = torch.half                                         # trace_info : t_1156
        ...
        if not args.loss_scale:                                                # trace_info : t_1157
            args.check_for_nan_in_loss_and_grad = False                        # trace_info : t_1158
            if args.rank == 0:                                                 # trace_info : t_1159
                print('WARNING: Setting args.check_for_nan_in_loss_and_grad to False since '# trace_info : t_1160
                      'dynamic loss scaling is being used')
    if args.bf16:                                                              # trace_info : t_1161
    ...
    if args.rank == 0:                                                         # trace_info : t_1162
        print('using {} for parameters ...'.format(args.params_dtype),         # trace_info : t_1163, t_1165
              flush=True)                                                      # trace_info : t_1164

    if args.dataloader_type is None:                                           # trace_info : t_1166
        args.dataloader_type = 'single'                                        # trace_info : t_1167

    # data
    assert args.num_dataset_builder_threads > 0                                # trace_info : t_1168

    # Consumed tokens.
    args.consumed_train_samples = 0                                            # trace_info : t_1169
    args.consumed_valid_samples = 0                                            # trace_info : t_1170
    ...
    args.variable_seq_lengths = False                                          # trace_info : t_1171

    # Iteration-based training.
    if args.train_iters:                                                       # trace_info : t_1172
        # If we use iteration-based training, make sure the
        # sample-based options are off.
        assert args.train_samples is None, \                                   # trace_info : t_1173
            'expected iteration-based training'
        assert args.lr_decay_samples is None, \                                # trace_info : t_1174
            'expected iteration-based learning rate decay'
        assert args.lr_warmup_samples == 0, \                                  # trace_info : t_1175
            'expected iteration-based learning rate warmup'
        assert args.rampup_batch_size is None, \                               # trace_info : t_1176
            'expected no batch-size rampup for iteration-based training'
        if args.lr_warmup_fraction is not None:                                # trace_info : t_1177
            assert args.lr_warmup_iters == 0, \                                # trace_info : t_1178
                'can only specify one of lr-warmup-fraction and lr-warmup-iters'

    # Sample-based training.
    if args.train_samples:                                                     # trace_info : t_1179
    ...
    if args.num_layers is not None:                                            # trace_info : t_1180
        assert args.encoder_num_layers is None, \                              # trace_info : t_1181
            'cannot have both num-layers and encoder-num-layers specified'
        args.encoder_num_layers = args.num_layers                              # trace_info : t_1182
    else:
    ...
    required_args = ['num_layers', 'hidden_size', 'num_attention_heads',       # trace_info : t_1183
                     'max_position_embeddings']
    for req_arg in required_args:                                              # trace_info : t_1184, t_1187, t_1190, t_1193, t_1196
        _check_arg_is_not_none(args, req_arg)                                  # trace_info : t_1185, t_1188, t_1191, t_1194

    # Checks.
    if args.ffn_hidden_size is None:                                           # trace_info : t_1197
        if args.swiglu:                                                        # trace_info : t_1198
            ...
            args.ffn_hidden_size = 4 * args.hidden_size                        # trace_info : t_1199

    if args.kv_channels is None:                                               # trace_info : t_1200
        assert args.hidden_size % args.num_attention_heads == 0                # trace_info : t_1201
        args.kv_channels = args.hidden_size // args.num_attention_heads        # trace_info : t_1202

    if args.seq_length is not None:                                            # trace_info : t_1203
        assert args.encoder_seq_length is None                                 # trace_info : t_1204
        args.encoder_seq_length = args.seq_length                              # trace_info : t_1205
    else:
        assert args.encoder_seq_length is not None
        args.seq_length = args.encoder_seq_length

    if args.seq_length is not None:                                            # trace_info : t_1206
        assert args.max_position_embeddings >= args.seq_length                 # trace_info : t_1207
    if args.decoder_seq_length is not None:                                    # trace_info : t_1208
        assert args.max_position_embeddings >= args.decoder_seq_length
    if args.lr is not None:                                                    # trace_info : t_1209
        assert args.min_lr <= args.lr                                          # trace_info : t_1210
    if args.save is not None:                                                  # trace_info : t_1211
        assert args.save_interval is not None
    # Mixed precision checks.
    if args.fp16_lm_cross_entropy:                                             # trace_info : t_1212
        assert args.fp16, 'lm cross entropy in fp16 only support in fp16 mode.'
    if args.fp32_residual_connection:                                          # trace_info : t_1213
        assert args.fp16 or args.bf16, \
            'residual connection in fp32 only supported when using fp16 or bf16.'

    if args.moe_grouped_gemm:                                                  # trace_info : t_1214
        assert args.bf16, 'Currently GroupedGEMM for MoE only supports bf16 dtype.'
        dc = torch.cuda.get_device_capability()
        assert dc[0] >= 8, "Unsupported compute capability for GroupedGEMM kernels."

    if args.weight_decay_incr_style == 'constant':                             # trace_info : t_1215
        assert args.start_weight_decay is None                                 # trace_info : t_1216
        assert args.end_weight_decay is None                                   # trace_info : t_1217
        args.start_weight_decay = args.weight_decay                            # trace_info : t_1218
        args.end_weight_decay = args.weight_decay                              # trace_info : t_1219
    else:
        assert args.start_weight_decay is not None
        assert args.end_weight_decay is not None

    TORCH_MAJOR = int(torch.__version__.split('.')[0])                         # trace_info : t_1220
    TORCH_MINOR = int(torch.__version__.split('.')[1])                         # trace_info : t_1221
    # Persistent fused layer norm.
    if TORCH_MAJOR < 1 or (TORCH_MAJOR == 1 and TORCH_MINOR < 11):             # trace_info : t_1222
    ...
    if args.distribute_saved_activations:                                      # trace_info : t_1223
    ...
    if args.recompute_granularity == 'selective':                              # trace_info : t_1224
    ...
    if args.tensor_model_parallel_size == 1:                                   # trace_info : t_1225
        args.sequence_parallel = False

    # disable async_tensor_model_parallel_allreduce when
    # model parallel memory optimization is enabled
    if args.sequence_parallel:                                                 # trace_info : t_1226
        args.async_tensor_model_parallel_allreduce = False

    if os.environ.get('CUDA_DEVICE_MAX_CONNECTIONS') != "1":                   # trace_info : t_1227
    ...
    if not args.add_bias_linear:                                               # trace_info : t_1228
        args.bias_gelu_fusion = False

    # Retro checks.
    if args.retro_add_retriever:                                               # trace_info : t_1229
    ...
    if args.decoupled_lr is not None or args.decoupled_min_lr is not None:     # trace_info : t_1230
        assert args.use_mcore_models, \
            '--decoupled-lr and --decoupled-min-lr only supported by Megatron Core, please add --use-mcore-models.'

    # Legacy RoPE arguments
    if args.use_rotary_position_embeddings:                                    # trace_info : t_1231
        args.position_embedding_type = 'rope'
    if args.rotary_interleaved and args.apply_rope_fusion:                     # trace_info : t_1232
        raise RuntimeError('--rotary-interleaved does not work with rope_fusion.')
    if args.rotary_interleaved and not args.use_mcore_models:                  # trace_info : t_1233
        raise RuntimeError('--rotary-interleaved only support Megatron Core, please add --use-mcore-models.')

    # Would just need to add 'NoPE' as a position_embedding_type to support this, but for now
    # don't allow it to keep things simple
    if not args.add_position_embedding and args.position_embedding_type != 'rope':# trace_info : t_1234
        raise RuntimeError('--no-position-embedding is deprecated, use --position-embedding-type')

    # MoE Spec check
    if args.num_experts is not None:                                           # trace_info : t_1235
        assert args.spec is None, "Model Spec must be None when using MoEs"

    # Expert parallelism check
    if args.expert_model_parallel_size  > 1:                                   # trace_info : t_1236
    ...
    if args.use_dist_ckpt and not args.use_mcore_models:                       # trace_info : t_1237
        raise RuntimeError('--use-dist-ckpt only support Megatron Core, please add --use-mcore-models.')

    # Data blend checks
    assert args.mock_data + \                                                  # trace_info : t_1238, t_1240, t_1242, t_1244
           bool(args.data_path) + \                                            # trace_info : t_1239
           any([args.train_data_path, args.valid_data_path, args.test_data_path]) \# trace_info : t_1241
           == 1, "A single data source must be provided"                       # trace_info : t_1243

    if args.use_tp_pp_dp_mapping:                                              # trace_info : t_1245
    ...
    # Print arguments.
    _print_args("arguments", args)                                             # trace_info : t_1246

    return args                                                                # trace_info : t_3099


def _print_args(title, args):
    """Print arguments."""
    if args.rank == 0:                                                         # trace_info : t_1247
        print(f'------------------------ {title} ------------------------',    # trace_info : t_1248, t_1250
              flush=True)                                                      # trace_info : t_1249
        str_list = []                                                          # trace_info : t_1251
        for arg in vars(args):                                                 # trace_info : t_1252, t_1255, t_1258, t_1261, t_1264, ...
            dots = '.' * (48 - len(arg))                                       # trace_info : t_1253, t_1256, t_1259, t_1262, t_1265, ...
            str_list.append('  {} {} {}'.format(arg, dots, getattr(args, arg)))# trace_info : t_1254, t_1257, t_1260, t_1263, t_1266, ...
        for arg in sorted(str_list, key=lambda x: x.lower()):                  # trace_info : t_2174, t_2175, t_2176, t_2177, t_2178, ...
            print(arg, flush=True)                                             # trace_info : t_2482, t_2484, t_2486, t_2488, t_2490, ...
        print(f'-------------------- end of {title} ---------------------',    # trace_info : t_3096, t_3098
              flush=True)                                                      # trace_info : t_3097


def _check_arg_is_not_none(args, arg):
    assert getattr(args, arg) is not None, '{} argument is None'.format(arg)   # trace_info : t_1186, t_1189, t_1192, t_1195


def core_transformer_config_from_args(args, config_class=None):

    # Config class.
    config_class = config_class or TransformerConfig                           # trace_info : t_9207

    # Translate args to core transformer configuration
    kw_args = {}                                                               # trace_info : t_9208
    for f in dataclasses.fields(config_class):                                 # trace_info : t_9209, t_9212, t_9215, t_9218, t_9221, ...
        if hasattr(args, f.name):                                              # trace_info : t_9210, t_9213, t_9216, t_9219, t_9222, ...
            kw_args[f.name] = getattr(args, f.name)                            # trace_info : t_9211, t_9214, t_9217, t_9220, t_9223, ...
    kw_args['persist_layer_norm'] = not args.no_persist_layer_norm             # trace_info : t_9513
    kw_args['layernorm_zero_centered_gamma'] = args.apply_layernorm_1p         # trace_info : t_9514
    kw_args['layernorm_epsilon'] = args.norm_epsilon                           # trace_info : t_9515
    kw_args['deallocate_pipeline_outputs'] = True                              # trace_info : t_9516
    kw_args['pipeline_dtype'] = args.params_dtype                              # trace_info : t_9517
    kw_args['batch_p2p_comm'] = not args.overlap_p2p_comm                      # trace_info : t_9518
    kw_args['num_moe_experts'] = args.num_experts                              # trace_info : t_9519
    kw_args['rotary_interleaved'] = args.rotary_interleaved                    # trace_info : t_9520
    if args.swiglu:                                                            # trace_info : t_9521
        kw_args['activation_func'] = F.silu
        kw_args['gated_linear_unit'] = True
        kw_args['bias_activation_fusion'] = args.bias_swiglu_fusion
    else:
        kw_args['bias_activation_fusion'] = args.bias_gelu_fusion              # trace_info : t_9522
    if args.squared_relu:                                                      # trace_info : t_9523
    ...
    if args.init_method_xavier_uniform:                                        # trace_info : t_9524
        kw_args['init_method'] = torch.nn.init.xavier_uniform_
        kw_args['scaled_init_method'] = torch.nn.init.xavier_uniform_
    if args.group_query_attention:                                             # trace_info : t_9525
        kw_args['num_query_groups'] = args.num_query_groups
    else:
        kw_args['num_query_groups'] = None                                     # trace_info : t_9526

    # Return config.
    return config_class(**kw_args)                                             # trace_info : t_9527
