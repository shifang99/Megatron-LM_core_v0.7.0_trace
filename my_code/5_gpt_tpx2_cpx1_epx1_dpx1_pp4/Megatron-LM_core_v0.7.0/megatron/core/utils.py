...
def ensure_divisibility(numerator, denominator):
    ...
    assert numerator % denominator == 0, "{} is not divisible by {}".format(numerator, denominator)# trace_info : t_9826, t_10095, t_10099, t_10103, t_10144, ...


def divide(numerator, denominator):
    """Ensure that numerator is divisible by the denominator and return
    the division value."""
    ensure_divisibility(numerator, denominator)                                # trace_info : t_9825, t_10094, t_10098, t_10102, t_10143, ...
    return numerator // denominator                                            # trace_info : t_9827, t_10096, t_10100, t_10104, t_10145, ...
...
def get_attr_wrapped_model(model, attr, allow_none=True, return_model_obj=False):
    ...
    if isinstance(model, list):                                                # trace_info : t_12441, t_16000, t_17878, t_17924, t_18130, ...
        raise RuntimeError("_get_attr_wrapped_model given a list of models")

    if allow_none:                                                             # trace_info : t_12442, t_16001, t_17879, t_17925, t_18131, ...

        def condition(model, attr):                                            # trace_info : t_17926, t_18132, t_18152, t_19446, t_19916, ...
            return not hasattr(model, attr)                                    # trace_info : t_17928, t_17932, t_17936, t_18134, t_18138, ...

    else:

        def condition(model, attr):                                            # trace_info : t_12443, t_16002, t_17880, t_19809, t_21610, ...
            return getattr(model, attr, None) is None                          # trace_info : t_12445, t_12449, t_16004, t_17882, t_19811, ...

    while condition(model, attr):                                              # trace_info : t_12444, t_12448, t_16003, t_17881, t_17927, ...
        if not hasattr(model, "module"):                                       # trace_info : t_12446, t_17929, t_17933, t_18135, t_18139, ...
            raise RuntimeError(f"_get_attr_wrapped_model couldn't find attribute {attr}")

        model = model.module                                                   # trace_info : t_12447, t_17930, t_17934, t_18136, t_18140, ...

    if return_model_obj:                                                       # trace_info : t_12450, t_16005, t_17883, t_17937, t_18143, ...
        return model                                                           # trace_info : t_19928, t_23656, t_27384
    return getattr(model, attr)                                                # trace_info : t_12451, t_16006, t_17884, t_17938, t_18144, ...


def get_model_type(model):
    return get_attr_wrapped_model(model, 'model_type')                         # trace_info : t_17923, t_19443, t_21653, t_23171, t_25381, ...


def get_model_config(model):
    return get_attr_wrapped_model(model, 'config', allow_none=False)           # trace_info : t_12440, t_15999, t_17877, t_19806, t_21607, ...
...
class GlobalMemoryBuffer:
    ...
    def __init__(self):
        self.buffer = {}                                                       # trace_info : t_8734

    def get_tensor(self, tensor_shape, dtype, name):
        required_len = reduce(operator.mul, tensor_shape, 1)                   # trace_info : t_18548, t_19043, t_22278, t_22771, t_26006, ...
        if (
            self.buffer.get((name, dtype), None) is None                       # trace_info : t_18549, t_19044, t_22279, t_22772, t_26007, ...
            or self.buffer[(name, dtype)].numel() < required_len               # trace_info : t_19045, t_22280, t_22773, t_26008, t_26501
        ):
            self.buffer[(name, dtype)] = torch.empty(                          # trace_info : t_18550, t_18552
                required_len, dtype=dtype, device=torch.cuda.current_device(), requires_grad=False# trace_info : t_18551
            )

        return self.buffer[(name, dtype)][0:required_len].view(*tensor_shape)  # trace_info : t_18553, t_19046, t_22281, t_22774, t_26009, ...
...
def _kernel_make_viewless_tensor(inp, requires_grad):
    ...
    field.
    '''
    out = torch.empty((1,), dtype=inp.dtype, device=inp.device, requires_grad=requires_grad,)# trace_info : t_18425, t_18746, t_18920, t_19239, t_22155, ...
    out.data = inp.data                                                        # trace_info : t_18426, t_18747, t_18921, t_19240, t_22156, ...
    return out                                                                 # trace_info : t_18427, t_18748, t_18922, t_19241, t_22157, ...
...
class MakeViewlessTensor(torch.autograd.Function):
    ...
    def forward(ctx, inp, requires_grad):
        return _kernel_make_viewless_tensor(inp, requires_grad)                # trace_info : t_18424, t_18745, t_18919, t_19238, t_22154, ...
...
def make_viewless_tensor(inp, requires_grad, keep_graph):
    ...
    if inp._base is None:                                                      # trace_info : t_18390, t_18421, t_18742, t_18884, t_18916, ...
        return inp                                                             # trace_info : t_18391, t_18885, t_19378, t_22121, t_22613, ...

    # create viewless tensor
    if keep_graph:                                                             # trace_info : t_18422, t_18743, t_18917, t_19236, t_22152, ...
        return MakeViewlessTensor.apply(inp, requires_grad)                    # trace_info : t_18423, t_18744, t_18918, t_19237, t_22153, ...
    else:
...
def init_method_normal(sigma):
    ...
    def init_(tensor):                                                         # trace_info : t_9680
        return torch.nn.init.normal_(tensor, mean=0.0, std=sigma)              # trace_info : t_9884, t_9904, t_10437, t_10766, t_11439, ...

    return init_                                                               # trace_info : t_9681


def scaled_init_method_normal(sigma, num_layers):
    """Init method based on N(0, sigma/sqrt(2*num_layers)."""
    std = sigma / math.sqrt(2.0 * num_layers)                                  # trace_info : t_9686

    def init_(tensor):                                                         # trace_info : t_9687
        return torch.nn.init.normal_(tensor, mean=0.0, std=std)                # trace_info : t_10295, t_10924, t_11297, t_11926

    return init_                                                               # trace_info : t_9688
...
class StragglerDetector:
    ...
    def null_method(self) -> None:
        ...
        pass                                                                   # trace_info : t_18191, t_18282, t_18294, t_19412, t_21921, ...
    ...
    def __enter__(self) -> "StragglerDetector":
        ...
        """
        self.start()                                                           # trace_info : t_18190, t_18293, t_21920, t_22023, t_25648, ...
        return self                                                            # trace_info : t_18192, t_18295, t_21922, t_22025, t_25650, ...
    ...
    def __call__(self, bdata: bool = False) -> "StragglerDetector":
        ...
        Returns:
            StragglerDetector: the instance
        """
        self.bdata = bdata                                                     # trace_info : t_18188, t_21918, t_25646
        return self                                                            # trace_info : t_18189, t_21919, t_25647
    ...
    def __exit__(
        ...
        ret = False                                                            # trace_info : t_18279, t_19409, t_22009, t_23137, t_25737, ...
        if ex_type is not None:                                                # trace_info : t_18280, t_19410, t_22010, t_23138, t_25738, ...
            err = traceback.format_exception(ex_tb)
            logger.warning(f"{str(ex_val)}\n{err}")
            ret = True
        self.stop()                                                            # trace_info : t_18281, t_19411, t_22011, t_23139, t_25739, ...
        return ret                                                             # trace_info : t_18283, t_19413, t_22013, t_23141, t_25741, ...
