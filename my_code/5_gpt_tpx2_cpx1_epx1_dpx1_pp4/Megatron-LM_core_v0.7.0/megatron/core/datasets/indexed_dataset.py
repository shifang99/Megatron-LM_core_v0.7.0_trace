...
class DType(Enum):
    ...
    def dtype_from_code(cls, value: int) -> Type[numpy.number]:
        ...
        """
        return getattr(numpy, cls(value).name)                                 # trace_info : t_16315
    ...
    def size(key: Union[int, Type[numpy.number]]) -> int:
        ...
        """
        if isinstance(key, int):                                               # trace_info : t_16317
            return DType.dtype_from_code(key)().itemsize
        elif numpy.number in key.__mro__:                                      # trace_info : t_16318
            return key().itemsize                                              # trace_info : t_16319
        else:
            raise ValueError
...
class _IndexReader(object):
    ...
    def __init__(self, idx_path: str, multimodal: bool) -> None:
        ...
        log_single_rank(logger, logging.INFO, f"Load the {type(self).__name__} from {idx_path}")# trace_info : t_16300

        with open(idx_path, "rb") as stream:                                   # trace_info : t_16308, t_16323
            header = stream.read(9)                                            # trace_info : t_16309
            assert header == _INDEX_HEADER, f"bad header, cannot read: {idx_path}"# trace_info : t_16310

            version = struct.unpack("<Q", stream.read(8))[0]                   # trace_info : t_16311
            assert version == 1, f"bad version, cannot read: {idx_path}"       # trace_info : t_16312

            code = struct.unpack("<B", stream.read(1))[0]                      # trace_info : t_16313
            self.dtype = DType.dtype_from_code(code)                           # trace_info : t_16314
            self.dtype_size = DType.size(self.dtype)                           # trace_info : t_16316

            self.sequence_count = struct.unpack("<Q", stream.read(8))[0]       # trace_info : t_16320
            self.document_count = struct.unpack("<Q", stream.read(8))[0]       # trace_info : t_16321

            offset = stream.tell()                                             # trace_info : t_16322

        self.bin_buffer_mmap = numpy.memmap(idx_path, mode="r", order="C")     # trace_info : t_16324
        self.bin_buffer = memoryview(self.bin_buffer_mmap)                     # trace_info : t_16325

        log_single_rank(logger, logging.INFO, f"\tExtract the sequence lengths")# trace_info : t_16326
        t_beg = time.time()                                                    # trace_info : t_16334
        self.sequence_lengths = numpy.frombuffer(                              # trace_info : t_16335, t_16337
            self.bin_buffer, dtype=numpy.int32, count=self.sequence_count, offset=offset# trace_info : t_16336
        )
        t_end = time.time()                                                    # trace_info : t_16338
        log_single_rank(logger, logging.DEBUG, f"\t> time elapsed: {t_end - t_beg:4f} seconds")# trace_info : t_16339

        log_single_rank(logger, logging.INFO, f"\tExtract the sequence pointers")# trace_info : t_16343
        t_beg = time.time()                                                    # trace_info : t_16351
        self.sequence_pointers = numpy.frombuffer(                             # trace_info : t_16352, t_16357
            self.bin_buffer,                                                   # trace_info : t_16353
            dtype=numpy.int64,                                                 # trace_info : t_16354
            count=self.sequence_count,                                         # trace_info : t_16355
            offset=offset + self.sequence_lengths.nbytes,                      # trace_info : t_16356
        )
        t_end = time.time()                                                    # trace_info : t_16358
        log_single_rank(logger, logging.DEBUG, f"\t> time elapsed: {t_end - t_beg:4f} seconds")# trace_info : t_16359

        log_single_rank(logger, logging.INFO, f"\tExtract the document indices")# trace_info : t_16363
        t_beg = time.time()                                                    # trace_info : t_16371
        self.document_indices = numpy.frombuffer(                              # trace_info : t_16372, t_16377
            self.bin_buffer,                                                   # trace_info : t_16373
            dtype=numpy.int64,                                                 # trace_info : t_16374
            count=self.document_count,                                         # trace_info : t_16375
            offset=offset + self.sequence_lengths.nbytes + self.sequence_pointers.nbytes,# trace_info : t_16376
        )
        t_end = time.time()                                                    # trace_info : t_16378
        log_single_rank(logger, logging.DEBUG, f"\t> time elapsed: {t_end - t_beg:4f} seconds")# trace_info : t_16379

        self.sequence_modes = None                                             # trace_info : t_16383
        if multimodal:                                                         # trace_info : t_16384
        ...
        assert self.sequence_lengths.shape[0] == len(self)                     # trace_info : t_16385
        assert self.sequence_lengths.shape[0] == self.sequence_count           # trace_info : t_16387
        assert self.sequence_lengths.shape[0] == self.document_indices[-1]     # trace_info : t_16388

        log_single_rank(logger, logging.INFO, f"> total number of sequences: {len(self)}")# trace_info : t_16389
        log_single_rank(                                                       # trace_info : t_16398, t_16402
            logger,                                                            # trace_info : t_16399
            logging.INFO,                                                      # trace_info : t_16400
            f"> total number of documents: {self.document_indices.shape[0] - 1}",# trace_info : t_16401
        )

    def __del__(self) -> None:
        """Clean up the object
        """
        if hasattr(self, "bin_buffer_mmap"):                                   # trace_info : t_28763
            self.bin_buffer_mmap._mmap.close()                                 # trace_info : t_28764
            del self.bin_buffer_mmap                                           # trace_info : t_28765
    ...
    def __len__(self) -> int:
        ...
        """
        return self.sequence_count                                             # trace_info : t_16386, t_16390
...
class IndexedDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, path_prefix: str, multimodal: bool = False, mmap: bool = True) -> None:
        super().__init__()                                                     # trace_info : t_16284
        self.path_prefix = None                                                # trace_info : t_16285
        self.multimodal = None                                                 # trace_info : t_16286
        self.mmap = None                                                       # trace_info : t_16287

        self.initialize(path_prefix, multimodal, mmap)                         # trace_info : t_16288
    ...
    def initialize(self, path_prefix: str, multimodal: bool, mmap: bool) -> None:
        ...
        """
        idx_path = get_idx_path(path_prefix)                                   # trace_info : t_16289
        bin_path = get_bin_path(path_prefix)                                   # trace_info : t_16291
        assert os.path.exists(idx_path) and os.path.exists(                    # trace_info : t_16293, t_16295
            bin_path                                                           # trace_info : t_16294
        ), f"One or both of the .idx and .bin files cannot be found at the path prefix {path_prefix}"

        self.path_prefix = path_prefix                                         # trace_info : t_16296
        self.multimodal = multimodal                                           # trace_info : t_16297
        self.mmap = mmap                                                       # trace_info : t_16298

        self.index = _IndexReader(idx_path, self.multimodal)                   # trace_info : t_16299
        self.bin_buffer = None                                                 # trace_info : t_16410
        self.bin_buffer_mmap = None                                            # trace_info : t_16411
        if mmap:                                                               # trace_info : t_16412
            self.bin_buffer_mmap = numpy.memmap(bin_path, mode="r", order="C") # trace_info : t_16413
            self.bin_buffer = memoryview(self.bin_buffer_mmap)                 # trace_info : t_16414
    ...
    def __getstate__(self) -> Tuple[str, bool, bool]:
        ...
        """
        return self.path_prefix, self.multimodal, self.mmap                    # trace_info : t_17269, t_17270, t_17311, t_17312, t_17353, ...
    ...
    def __del__(self) -> None:
        """Clean up the object
        """
        if self.bin_buffer_mmap is not None:                                   # trace_info : t_28759
            self.bin_buffer_mmap._mmap.close()                                 # trace_info : t_28760
        del self.bin_buffer_mmap                                               # trace_info : t_28761
        del self.index                                                         # trace_info : t_28762
    ...
    def sequence_lengths(self) -> numpy.ndarray:
        ...
        """
        return self.index.sequence_lengths                                     # trace_info : t_16417
...
def get_idx_path(path_prefix: str) -> str:
    ...
    """
    return path_prefix + ".idx"                                                # trace_info : t_16290
...
def get_bin_path(path_prefix: str) -> str:
    ...
    """
    return path_prefix + ".bin"                                                # trace_info : t_16292
