...
class LanguageModule(MegatronModule):
    ...
    def __init__(self, config: TransformerConfig) -> None:
        super().__init__(config=config)                                        # trace_info : t_9771
    ...
    def setup_embeddings_and_output_layer(self) -> None:
        ...
        if self.pre_process:                                                   # trace_info : t_11971
            self.embedding.word_embeddings.weight.is_embedding_or_output_parameter = True# trace_info : t_11972
        if self.post_process and self.output_layer.weight is not None:         # trace_info : t_11973
            self.output_layer.weight.is_embedding_or_output_parameter = True

        if not self.share_embeddings_and_output_weights:                       # trace_info : t_11974
            return

        if self.pre_process and self.post_process:                             # trace_info : t_11975
        ...
        if self.pre_process and not self.post_process:                         # trace_info : t_11976
            assert parallel_state.is_pipeline_first_stage()                    # trace_info : t_11977
            self.shared_embedding_or_output_weight().shared_embedding = True   # trace_info : t_11986

        if self.post_process and not self.pre_process:                         # trace_info : t_11989
        ...
        # values.
        if torch.distributed.is_initialized():                                 # trace_info : t_11990
            if parallel_state.is_rank_in_embedding_group():                    # trace_info : t_11991
                weight = self.shared_embedding_or_output_weight()              # trace_info : t_12005
                weight.data = weight.data.cuda()                               # trace_info : t_12008
                torch.distributed.all_reduce(                                  # trace_info : t_12009, t_12013
                    weight.data, group=parallel_state.get_embedding_group()    # trace_info : t_12010
                )
    ...
    def shared_embedding_or_output_weight(self) -> Tensor:
        ...
        """
        if self.pre_process:                                                   # trace_info : t_11987, t_12006, t_19931, t_23659, t_27387
            return self.embedding.word_embeddings.weight                       # trace_info : t_11988, t_12007, t_19932, t_23660, t_27388
