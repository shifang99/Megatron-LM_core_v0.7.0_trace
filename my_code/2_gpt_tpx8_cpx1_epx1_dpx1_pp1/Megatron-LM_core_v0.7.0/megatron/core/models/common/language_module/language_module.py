...
class LanguageModule(MegatronModule):
    ...
    def __init__(self, config: TransformerConfig) -> None:
        super().__init__(config=config)                                        # trace_info : t_11272
    ...
    def compute_language_model_loss(self, labels: Tensor, logits: Tensor) -> Tensor:
        ...
        """
        # [b s] => [s b]
        labels = labels.transpose(0, 1).contiguous()                           # trace_info : t_21169, t_24779, t_28389
        loss = tensor_parallel.vocab_parallel_cross_entropy(logits.float(), labels)# trace_info : t_21170, t_24780, t_28390

        # [s b] => [b, s]
        loss = loss.transpose(0, 1).contiguous()                               # trace_info : t_21234, t_24844, t_28454
        return loss                                                            # trace_info : t_21235, t_24845, t_28455
    ...
    def setup_embeddings_and_output_layer(self) -> None:
        ...
        if self.pre_process:                                                   # trace_info : t_13552
            self.embedding.word_embeddings.weight.is_embedding_or_output_parameter = True# trace_info : t_13553
        if self.post_process and self.output_layer.weight is not None:         # trace_info : t_13554
            self.output_layer.weight.is_embedding_or_output_parameter = True

        if not self.share_embeddings_and_output_weights:                       # trace_info : t_13555
            return

        if self.pre_process and self.post_process:                             # trace_info : t_13556
            # Zero out wgrad if sharing embeddings between two layers on same
            # pipeline stage to make sure grad accumulation into main_grad is
            # correct and does not include garbage values (e.g., from torch.empty).
            self.shared_embedding_or_output_weight().zero_out_wgrad = True     # trace_info : t_13557
            return                                                             # trace_info : t_13560
    ...
    def shared_embedding_or_output_weight(self) -> Tensor:
        ...
        """
        if self.pre_process:                                                   # trace_info : t_13558, t_21113, t_24723, t_28333
            return self.embedding.word_embeddings.weight                       # trace_info : t_13559, t_21114, t_24724, t_28334
