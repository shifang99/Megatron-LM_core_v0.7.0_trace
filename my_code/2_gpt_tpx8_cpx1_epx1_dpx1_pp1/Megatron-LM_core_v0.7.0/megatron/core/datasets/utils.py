...
def compile_helpers():
    ...
    """
    import os                                                                  # trace_info : t_10384
    import subprocess                                                          # trace_info : t_10385

    command = ["make", "-C", os.path.abspath(os.path.dirname(__file__))]       # trace_info : t_10386
    if subprocess.run(command).returncode != 0:                                # trace_info : t_10387
        import sys
...
def log_single_rank(logger: logging.Logger, *args: Any, rank: int = 0, **kwargs: Any):
    ...
    """
    if torch.distributed.is_initialized():                                     # trace_info : t_17991, t_18026, t_18102, t_18128, t_18141, ...
        if torch.distributed.get_rank() == rank:                               # trace_info : t_17992, t_18027, t_18103, t_18129, t_18142, ...
            logger.log(*args, **kwargs)                                        # trace_info : t_17993, t_18028, t_18104, t_18130, t_18143, ...
    else:
...
def normalize(weights: List[float]) -> List[float]:
    ...
    """
    w = numpy.array(weights, dtype=numpy.float64)                              # trace_info : t_17957
    w_sum = numpy.sum(w)                                                       # trace_info : t_17958
    w = (w / w_sum).tolist()                                                   # trace_info : t_17959
    return w                                                                   # trace_info : t_17960
...
def get_blend_from_list(
    ...
    """
    if blend is None:                                                          # trace_info : t_17903, t_17910, t_17913, t_17916
        return None                                                            # trace_info : t_17911, t_17914, t_17917

    if len(blend) % 2 == 1:                                                    # trace_info : t_17904
        weight_per_dataset = None                                              # trace_info : t_17905
        raw_prefix_per_dataset = blend                                         # trace_info : t_17906
    else:
    ...
    prefix_per_dataset = [rppd.strip() for rppd in raw_prefix_per_dataset]     # trace_info : t_17907

    return prefix_per_dataset, weight_per_dataset                              # trace_info : t_17908
