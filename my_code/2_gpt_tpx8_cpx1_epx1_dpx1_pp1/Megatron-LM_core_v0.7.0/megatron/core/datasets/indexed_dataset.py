...
class DType(Enum):
    ...
    def dtype_from_code(cls, value: int) -> Type[numpy.number]:
        ...
        """
        return getattr(numpy, cls(value).name)                                 # trace_info : t_18116
    ...
    def size(key: Union[int, Type[numpy.number]]) -> int:
        ...
        """
        if isinstance(key, int):                                               # trace_info : t_18118
            return DType.dtype_from_code(key)().itemsize
        elif numpy.number in key.__mro__:                                      # trace_info : t_18119
            return key().itemsize                                              # trace_info : t_18120
        else:
            raise ValueError
...
class _IndexReader(object):
    ...
    def __init__(self, idx_path: str, multimodal: bool) -> None:
        ...
        log_single_rank(logger, logging.INFO, f"Load the {type(self).__name__} from {idx_path}")# trace_info : t_18101

        with open(idx_path, "rb") as stream:                                   # trace_info : t_18109, t_18124
            header = stream.read(9)                                            # trace_info : t_18110
            assert header == _INDEX_HEADER, f"bad header, cannot read: {idx_path}"# trace_info : t_18111

            version = struct.unpack("<Q", stream.read(8))[0]                   # trace_info : t_18112
            assert version == 1, f"bad version, cannot read: {idx_path}"       # trace_info : t_18113

            code = struct.unpack("<B", stream.read(1))[0]                      # trace_info : t_18114
            self.dtype = DType.dtype_from_code(code)                           # trace_info : t_18115
            self.dtype_size = DType.size(self.dtype)                           # trace_info : t_18117

            self.sequence_count = struct.unpack("<Q", stream.read(8))[0]       # trace_info : t_18121
            self.document_count = struct.unpack("<Q", stream.read(8))[0]       # trace_info : t_18122

            offset = stream.tell()                                             # trace_info : t_18123

        self.bin_buffer_mmap = numpy.memmap(idx_path, mode="r", order="C")     # trace_info : t_18125
        self.bin_buffer = memoryview(self.bin_buffer_mmap)                     # trace_info : t_18126

        log_single_rank(logger, logging.INFO, f"\tExtract the sequence lengths")# trace_info : t_18127
        t_beg = time.time()                                                    # trace_info : t_18135
        self.sequence_lengths = numpy.frombuffer(                              # trace_info : t_18136, t_18138
            self.bin_buffer, dtype=numpy.int32, count=self.sequence_count, offset=offset# trace_info : t_18137
        )
        t_end = time.time()                                                    # trace_info : t_18139
        log_single_rank(logger, logging.DEBUG, f"\t> time elapsed: {t_end - t_beg:4f} seconds")# trace_info : t_18140

        log_single_rank(logger, logging.INFO, f"\tExtract the sequence pointers")# trace_info : t_18144
        t_beg = time.time()                                                    # trace_info : t_18152
        self.sequence_pointers = numpy.frombuffer(                             # trace_info : t_18153, t_18158
            self.bin_buffer,                                                   # trace_info : t_18154
            dtype=numpy.int64,                                                 # trace_info : t_18155
            count=self.sequence_count,                                         # trace_info : t_18156
            offset=offset + self.sequence_lengths.nbytes,                      # trace_info : t_18157
        )
        t_end = time.time()                                                    # trace_info : t_18159
        log_single_rank(logger, logging.DEBUG, f"\t> time elapsed: {t_end - t_beg:4f} seconds")# trace_info : t_18160

        log_single_rank(logger, logging.INFO, f"\tExtract the document indices")# trace_info : t_18164
        t_beg = time.time()                                                    # trace_info : t_18172
        self.document_indices = numpy.frombuffer(                              # trace_info : t_18173, t_18178
            self.bin_buffer,                                                   # trace_info : t_18174
            dtype=numpy.int64,                                                 # trace_info : t_18175
            count=self.document_count,                                         # trace_info : t_18176
            offset=offset + self.sequence_lengths.nbytes + self.sequence_pointers.nbytes,# trace_info : t_18177
        )
        t_end = time.time()                                                    # trace_info : t_18179
        log_single_rank(logger, logging.DEBUG, f"\t> time elapsed: {t_end - t_beg:4f} seconds")# trace_info : t_18180

        self.sequence_modes = None                                             # trace_info : t_18184
        if multimodal:                                                         # trace_info : t_18185
        ...
        assert self.sequence_lengths.shape[0] == len(self)                     # trace_info : t_18186
        assert self.sequence_lengths.shape[0] == self.sequence_count           # trace_info : t_18188
        assert self.sequence_lengths.shape[0] == self.document_indices[-1]     # trace_info : t_18189

        log_single_rank(logger, logging.INFO, f"> total number of sequences: {len(self)}")# trace_info : t_18190
        log_single_rank(                                                       # trace_info : t_18199, t_18203
            logger,                                                            # trace_info : t_18200
            logging.INFO,                                                      # trace_info : t_18201
            f"> total number of documents: {self.document_indices.shape[0] - 1}",# trace_info : t_18202
        )

    def __del__(self) -> None:
        """Clean up the object
        """
        if hasattr(self, "bin_buffer_mmap"):                                   # trace_info : t_30206
            self.bin_buffer_mmap._mmap.close()                                 # trace_info : t_30207
            del self.bin_buffer_mmap                                           # trace_info : t_30208
    ...
    def __len__(self) -> int:
        ...
        """
        return self.sequence_count                                             # trace_info : t_18187, t_18191
...
class IndexedDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, path_prefix: str, multimodal: bool = False, mmap: bool = True) -> None:
        super().__init__()                                                     # trace_info : t_18085
        self.path_prefix = None                                                # trace_info : t_18086
        self.multimodal = None                                                 # trace_info : t_18087
        self.mmap = None                                                       # trace_info : t_18088

        self.initialize(path_prefix, multimodal, mmap)                         # trace_info : t_18089
    ...
    def initialize(self, path_prefix: str, multimodal: bool, mmap: bool) -> None:
        ...
        """
        idx_path = get_idx_path(path_prefix)                                   # trace_info : t_18090
        bin_path = get_bin_path(path_prefix)                                   # trace_info : t_18092
        assert os.path.exists(idx_path) and os.path.exists(                    # trace_info : t_18094, t_18096
            bin_path                                                           # trace_info : t_18095
        ), f"One or both of the .idx and .bin files cannot be found at the path prefix {path_prefix}"

        self.path_prefix = path_prefix                                         # trace_info : t_18097
        self.multimodal = multimodal                                           # trace_info : t_18098
        self.mmap = mmap                                                       # trace_info : t_18099

        self.index = _IndexReader(idx_path, self.multimodal)                   # trace_info : t_18100
        self.bin_buffer = None                                                 # trace_info : t_18211
        self.bin_buffer_mmap = None                                            # trace_info : t_18212
        if mmap:                                                               # trace_info : t_18213
            self.bin_buffer_mmap = numpy.memmap(bin_path, mode="r", order="C") # trace_info : t_18214
            self.bin_buffer = memoryview(self.bin_buffer_mmap)                 # trace_info : t_18215
    ...
    def __getstate__(self) -> Tuple[str, bool, bool]:
        ...
        """
        return self.path_prefix, self.multimodal, self.mmap                    # trace_info : t_19070, t_19071, t_19112, t_19113, t_19154, ...
    ...
    def __del__(self) -> None:
        """Clean up the object
        """
        if self.bin_buffer_mmap is not None:                                   # trace_info : t_30202
            self.bin_buffer_mmap._mmap.close()                                 # trace_info : t_30203
        del self.bin_buffer_mmap                                               # trace_info : t_30204
        del self.index                                                         # trace_info : t_30205
    ...
    def sequence_lengths(self) -> numpy.ndarray:
        ...
        """
        return self.index.sequence_lengths                                     # trace_info : t_18218
...
def get_idx_path(path_prefix: str) -> str:
    ...
    """
    return path_prefix + ".idx"                                                # trace_info : t_18091
...
def get_bin_path(path_prefix: str) -> str:
    ...
    """
    return path_prefix + ".bin"                                                # trace_info : t_18093
