...
class MegatronTokenizer(ABC):
    ...
    def __init__(self, *tokenizer_paths: str, **tokenizer_options: Any):
        ...
        self.unique_identifiers = OrderedDict()                                # trace_info : t_3151
        self.unique_identifiers["class"] = type(self).__name__                 # trace_info : t_3152
        self.unique_identifiers["tokenizer_path"] = list(tokenizer_paths)      # trace_info : t_3153
        for option in tokenizer_options:                                       # trace_info : t_3154
            self.unique_identifiers[option] = str(tokenizer_options[option])

        self.unique_description = json.dumps(self.unique_identifiers, indent=4)# trace_info : t_3155

        super().__init__()                                                     # trace_info : t_3156
    ...
    def pad(self):
        ...
        Raises:
            NotImplementedError: Non-abstract, optional attribute
        """
        raise NotImplementedError("{} has no attribute 'pad'".format(type(self).__name__))# trace_info : t_18321, t_18536, t_18751
