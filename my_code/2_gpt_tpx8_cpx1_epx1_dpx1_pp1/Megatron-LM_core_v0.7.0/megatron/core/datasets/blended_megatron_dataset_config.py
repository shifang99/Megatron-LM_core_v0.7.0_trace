...
class BlendedMegatronDatasetConfig:
    ...
    def __post_init__(self) -> None:
        ...
        """
        if self.blend_per_split is not None and any(self.blend_per_split):     # trace_info : t_17946
            ...
            assert self.split is not None, "split must be provided in absence of blend_per_split"# trace_info : t_17947
            split_vector = parse_and_normalize_split(self.split)               # trace_info : t_17948
            self.split_matrix = convert_split_vector_to_split_matrix(split_vector)# trace_info : t_17962
            log_single_rank(logger, logging.INFO, f"Let split_matrix = {self.split_matrix}")# trace_info : t_17990
            if self.blend is not None:                                         # trace_info : t_17998
                assert self.blend[1] is None or len(self.blend[0]) == len(     # trace_info : t_17999
                    self.blend[1]
...
def parse_and_normalize_split(split: str) -> List[float]:
    ...
    """
    split = list(map(float, re.findall(r"[.0-9]+", split)))                    # trace_info : t_17949
    split = split + [0.0 for _ in range(len(Split) - len(split))]              # trace_info : t_17950

    assert len(split) == len(Split)                                            # trace_info : t_17951
    assert all(map(lambda _: _ >= 0.0, split))                                 # trace_info : t_17952, t_17953, t_17954, t_17955

    split = normalize(split)                                                   # trace_info : t_17956

    return split                                                               # trace_info : t_17961
...
def convert_split_vector_to_split_matrix(
    ...
    """
    if vector_b is None:                                                       # trace_info : t_17963
        vector_b = vector_a                                                    # trace_info : t_17964

    # [.900, .090, .010] -> [0.00, .900, .990, 100]
    expansion_a = functools.reduce(lambda a, b: a + [a[len(a) - 1] + b], [[0], *vector_a])# trace_info : t_17965, t_17966, t_17967, t_17968
    expansion_b = functools.reduce(lambda a, b: a + [a[len(a) - 1] + b], [[0], *vector_b])# trace_info : t_17969, t_17970, t_17971, t_17972

    # [0.00, .900, .990, 100.0] -> [(0.00, .900), (.900, .990), (.990, 100)]
    bookends_a = list(zip(expansion_a[:-1], expansion_a[1:]))                  # trace_info : t_17973
    bookends_b = list(zip(expansion_b[:-1], expansion_b[1:]))                  # trace_info : t_17974

    # gather per-split overlap or None
    matrix = []                                                                # trace_info : t_17975
    for bookend_a, bookend_b in zip(bookends_a, bookends_b):                   # trace_info : t_17976, t_17980, t_17984, t_17988
        if min(bookend_a[1], bookend_b[1]) <= max(bookend_a[0], bookend_b[0]): # trace_info : t_17977, t_17981, t_17985
            overlap = None
        else:
            overlap = (max(bookend_a[0], bookend_b[0]), min(bookend_a[1], bookend_b[1]))# trace_info : t_17978, t_17982, t_17986
        matrix.append(overlap)                                                 # trace_info : t_17979, t_17983, t_17987

    return matrix                                                              # trace_info : t_17989
