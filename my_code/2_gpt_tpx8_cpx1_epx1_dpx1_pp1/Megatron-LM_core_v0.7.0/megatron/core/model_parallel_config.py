...
class ModelParallelConfig:
    ...
    def __post_init__(self):
        ...
        """
        if self.sequence_parallel:                                             # trace_info : t_11146
            if self.tensor_model_parallel_size <= 1:
                raise ValueError("Can not use sequence paralllelism without tensor parallelism")

        if self.pipeline_model_parallel_size > 1:                              # trace_info : t_11147
            if self.pipeline_dtype is None:
                raise ValueError(
                    "When using pipeline parallelism, pipeline_dtype must be specified"
                )

        if self.autocast_dtype is None:                                        # trace_info : t_11148
            self.autocast_dtype = self.params_dtype                            # trace_info : t_11149

        if self.defer_embedding_wgrad_compute and self.pipeline_model_parallel_size == 1:# trace_info : t_11150
            raise ValueError(
                "Cannot defer embedding wgrad compute when pipeline model parallel is not used"
            )

        if self.defer_embedding_wgrad_compute and not self.gradient_accumulation_fusion:# trace_info : t_11151
            raise ValueError(
                "Cannot defer embedding wgrad compute when gradient accumulation fusion is not used"
            )

        if self.expert_model_parallel_size > 1 and self.tensor_model_parallel_size > 1:# trace_info : t_11152
