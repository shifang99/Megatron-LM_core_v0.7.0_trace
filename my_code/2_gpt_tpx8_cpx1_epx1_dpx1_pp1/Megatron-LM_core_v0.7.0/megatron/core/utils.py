...
def ensure_divisibility(numerator, denominator):
    ...
    assert numerator % denominator == 0, "{} is not divisible by {}".format(numerator, denominator)# trace_info : t_11327, t_11596, t_11600, t_11604, t_11645, ...


def divide(numerator, denominator):
    """Ensure that numerator is divisible by the denominator and return
    the division value."""
    ensure_divisibility(numerator, denominator)                                # trace_info : t_11326, t_11595, t_11599, t_11603, t_11644, ...
    return numerator // denominator                                            # trace_info : t_11328, t_11597, t_11601, t_11605, t_11646, ...
...
def get_attr_wrapped_model(model, attr, allow_none=True, return_model_obj=False):
    ...
    if isinstance(model, list):                                                # trace_info : t_14020, t_17801, t_19691, t_19711, t_19855, ...
        raise RuntimeError("_get_attr_wrapped_model given a list of models")

    if allow_none:                                                             # trace_info : t_14021, t_17802, t_19692, t_19712, t_19856, ...

        def condition(model, attr):                                            # trace_info : t_19713, t_19857, t_19879, t_21329, t_23325, ...
            return not hasattr(model, attr)                                    # trace_info : t_19715, t_19719, t_19723, t_19859, t_19863, ...

    else:

        def condition(model, attr):                                            # trace_info : t_14022, t_17803, t_19693, t_21430, t_23305, ...
            return getattr(model, attr, None) is None                          # trace_info : t_14024, t_14028, t_17805, t_19695, t_21432, ...

    while condition(model, attr):                                              # trace_info : t_14023, t_14027, t_17804, t_19694, t_19714, ...
        if not hasattr(model, "module"):                                       # trace_info : t_14025, t_19716, t_19720, t_19860, t_19864, ...
            raise RuntimeError(f"_get_attr_wrapped_model couldn't find attribute {attr}")

        model = model.module                                                   # trace_info : t_14026, t_19717, t_19721, t_19861, t_19865, ...

    if return_model_obj:                                                       # trace_info : t_14029, t_17806, t_19696, t_19724, t_19868, ...
        return model
    return getattr(model, attr)                                                # trace_info : t_14030, t_17807, t_19697, t_19725, t_19869, ...


def get_model_type(model):
    return get_attr_wrapped_model(model, 'model_type')                         # trace_info : t_19710, t_21326, t_23322, t_24936, t_26932, ...


def get_model_config(model):
    return get_attr_wrapped_model(model, 'config', allow_none=False)           # trace_info : t_14019, t_17800, t_19690, t_21427, t_23302, ...
...
class GlobalMemoryBuffer:
    ...
    def __init__(self):
        self.buffer = {}                                                       # trace_info : t_10234

    def get_tensor(self, tensor_shape, dtype, name):
        required_len = reduce(operator.mul, tensor_shape, 1)                   # trace_info : t_20282, t_20769, t_23894, t_24379, t_27504, ...
        if (
            self.buffer.get((name, dtype), None) is None                       # trace_info : t_20283, t_20770, t_23895, t_24380, t_27505, ...
            or self.buffer[(name, dtype)].numel() < required_len               # trace_info : t_20771, t_23896, t_24381, t_27506, t_27991
        ):
            self.buffer[(name, dtype)] = torch.empty(                          # trace_info : t_20284, t_20286
                required_len, dtype=dtype, device=torch.cuda.current_device(), requires_grad=False# trace_info : t_20285
            )

        return self.buffer[(name, dtype)][0:required_len].view(*tensor_shape)  # trace_info : t_20287, t_20772, t_23897, t_24382, t_27507, ...
...
def _kernel_make_viewless_tensor(inp, requires_grad):
    ...
    field.
    '''
    out = torch.empty((1,), dtype=inp.dtype, device=inp.device, requires_grad=requires_grad,)# trace_info : t_20159, t_20472, t_20646, t_20957, t_23771, ...
    out.data = inp.data                                                        # trace_info : t_20160, t_20473, t_20647, t_20958, t_23772, ...
    return out                                                                 # trace_info : t_20161, t_20474, t_20648, t_20959, t_23773, ...
...
class MakeViewlessTensor(torch.autograd.Function):
    ...
    def forward(ctx, inp, requires_grad):
        return _kernel_make_viewless_tensor(inp, requires_grad)                # trace_info : t_20158, t_20471, t_20645, t_20956, t_23770, ...
...
def make_viewless_tensor(inp, requires_grad, keep_graph):
    ...
    if inp._base is None:                                                      # trace_info : t_20124, t_20155, t_20468, t_20610, t_20642, ...
        return inp                                                             # trace_info : t_20125, t_20611, t_21096, t_23737, t_24221, ...

    # create viewless tensor
    if keep_graph:                                                             # trace_info : t_20156, t_20469, t_20643, t_20954, t_23768, ...
        return MakeViewlessTensor.apply(inp, requires_grad)                    # trace_info : t_20157, t_20470, t_20644, t_20955, t_23769, ...
    else:
...
def init_method_normal(sigma):
    ...
    def init_(tensor):                                                         # trace_info : t_11181
        return torch.nn.init.normal_(tensor, mean=0.0, std=sigma)              # trace_info : t_11385, t_11405, t_11938, t_12267, t_12940, ...

    return init_                                                               # trace_info : t_11182


def scaled_init_method_normal(sigma, num_layers):
    """Init method based on N(0, sigma/sqrt(2*num_layers)."""
    std = sigma / math.sqrt(2.0 * num_layers)                                  # trace_info : t_11187

    def init_(tensor):                                                         # trace_info : t_11188
        return torch.nn.init.normal_(tensor, mean=0.0, std=std)                # trace_info : t_11796, t_12425, t_12798, t_13427

    return init_                                                               # trace_info : t_11189
...
class StragglerDetector:
    ...
    def null_method(self) -> None:
        ...
        pass                                                                   # trace_info : t_19918, t_20016, t_20028, t_21267, t_23530, ...
    ...
    def __enter__(self) -> "StragglerDetector":
        ...
        """
        self.start()                                                           # trace_info : t_19917, t_20027, t_23529, t_23639, t_27139, ...
        return self                                                            # trace_info : t_19919, t_20029, t_23531, t_23641, t_27141, ...
    ...
    def __call__(self, bdata: bool = False) -> "StragglerDetector":
        ...
        Returns:
            StragglerDetector: the instance
        """
        self.bdata = bdata                                                     # trace_info : t_19915, t_23527, t_27137
        return self                                                            # trace_info : t_19916, t_23528, t_27138
    ...
    def __exit__(
        ...
        ret = False                                                            # trace_info : t_20013, t_21264, t_23625, t_24874, t_27235, ...
        if ex_type is not None:                                                # trace_info : t_20014, t_21265, t_23626, t_24875, t_27236, ...
            err = traceback.format_exception(ex_tb)
            logger.warning(f"{str(ex_val)}\n{err}")
            ret = True
        self.stop()                                                            # trace_info : t_20015, t_21266, t_23627, t_24876, t_27237, ...
        return ret                                                             # trace_info : t_20017, t_21268, t_23629, t_24878, t_27239, ...
